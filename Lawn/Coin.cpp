...
static constexpr int AUTO_COLLECT_DELAY_FRAMES = 30;
...
void Coin::UpdateFall()
{
    ...
    else
    {
            ...
            mBoard->mChallenge->mChallengeState == ChallengeState::STATECHALLENGE_LAST_STAND_ONSLAUGHT)
        {
            ...
            {
                mDisappearCounter++;
                if (mBoard && mBoard->mAutoCollectSunEnabled && IsSun() && !mIsBeingCollected
                    && mDisappearCounter >= AUTO_COLLECT_DELAY_FRAMES)
                {
                    Collect();
                }
                else if (mDisappearCounter >= GetDisappearTime())
                {
                    StartFade();
                }
            }
