...
void PlayerInfo::SyncDetails(DataSync& theSync)
{
	...
	{
		PottedPlant& thePlant = mPottedPlant[i];
		theSync.SyncLong((int&)thePlant.mSeedType);
		theSync.SyncLong((int&)thePlant.mWhichZenGarden);
		theSync.SyncLong(thePlant.mX);
		theSync.SyncLong(thePlant.mY);
		theSync.SyncLong((int&)thePlant.mFacing);
		theSync.SyncBytes(&thePlant.mLastWateredTime, 8);
		theSync.SyncLong((int&)thePlant.mDrawVariation);
		theSync.SyncLong((int&)thePlant.mPlantAge);
		theSync.SyncLong(thePlant.mTimesFed);
		theSync.SyncLong(thePlant.mFeedingsPerGrow);
		theSync.SyncLong((int&)thePlant.mPlantNeed);
		theSync.SyncBytes(&thePlant.mLastNeedFulfilledTime, 8);
		theSync.SyncBytes(&thePlant.mLastFertilizedTime, 8);
		theSync.SyncBytes(&thePlant.mLastChocolateTime, 8);
		theSync.SyncBytes(&thePlant.mFutureAttribute[0], 8);
	}
}
