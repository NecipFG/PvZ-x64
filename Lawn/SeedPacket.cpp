...
void SeedPacket::PickNextSlotMachineSeed() {
  ...
  for (int i = 0; i < LENGTH(SLOT_SEED_TYPES); i++) {
    ...
    if (aSeedType == SeedType::SEED_PEASHOOTER) {
      aWeight =
          TodAnimateCurve(0, 5, aPeasCount, 200, 100, TodCurves::CURVE_LINEAR);
    ...
    if (mIndex == 2 && aSeedType != SeedType::SEED_SLOT_MACHINE_DIAMOND) {
      if (aSeedType ==
              mBoard->mSeedBank->mSeedPackets[0].mSlotMachiningNextSeed ||
          aSeedType ==
              mBoard->mSeedBank->mSeedPackets[1].mSlotMachiningNextSeed) {
        aWeight += aWeight / 2;
      }
    }
  ...
  mSlotMachiningNextSeed =
      (SeedType)TodPickFromWeightedArray(aSeedWeightArray, aSeedsCount);
}
...
void SeedPacket::FlashIfReady() {
  ...
  if (!mBoard->HasConveyorBeltSeedBank()) {
    int aRenderPosition =
        Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_UI_BOTTOM, 0, 2);
    mApp->AddTodParticle(mX + mBoard->mSeedBank->mX, mY + mBoard->mSeedBank->mY,
                         aRenderPosition,
                         ParticleEffect::PARTICLE_SEED_PACKET_FLASH);
  }

  if (mBoard->mTutorialState ==
      TutorialState::TUTORIAL_LEVEL_1_REFRESH_PEASHOOTER) {
    mBoard->SetTutorialState(
        TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER);
  } else if (mBoard->mTutorialState ==
...
void SeedPacket::Update() {
  ...
  if (mSlotMachineCountDown > 0) {
    ...
    float aFlipsPerSecont =
        TodAnimateCurveFloat(SLOT_MACHINE_TIME, 0, mSlotMachineCountDown, 6.0f,
                             2.0f, TodCurves::CURVE_LINEAR);
...
void SeedPacketDrawSeed(Graphics *g, float x, float y, SeedType theSeedType,
                        SeedType theImitaterType, float theOffsetX,
  ...
  if (theSeedType == SeedType::SEED_IMITATER &&
    ...
    if (theImitaterType == SeedType::SEED_HYPNOSHROOM ||
        ...
        theImitaterType == SeedType::SEED_GARLIC ||
  ...
  if (aSeedType == SeedType::SEED_POTATOMINE && g->mScaleX <= 1.0f) {
    ...
    Plant::DrawSeedType(&aSeedG, theSeedType, theImitaterType,
                        DrawVariation::VARIATION_NORMAL, x + theOffsetX,
                        y + theOffsetY);
  }
}

// 0x4876F0
void DrawSeedPacket(Graphics *g, float x, float y, SeedType theSeedType,
                    SeedType theImitaterType, float thePercentDark,
  ...
  int aPacketBackground =
      ...
      : (Plant::IsUpgrade(aSeedType) && (!gLawnApp->mBoard || gLawnApp->mBoard->mLevel != 60)) ? 1
  ...
  if (g->mScaleX > 1) {
    TodDrawImageCelScaledF(g, Sexy::IMAGE_SEEDPACKET_LARGER, x, y, 0, 0,
                           g->mScaleX * 0.5f, g->mScaleY * 0.5f);
  } else {
    TodDrawImageCelScaledF(g, Sexy::IMAGE_SEEDS, x, y, aPacketBackground, 0,
                           g->mScaleX, g->mScaleY);
  }
  ...
  if (((LawnApp *)gSexyAppBase)->mGameMode ==
      GameMode::GAMEMODE_CHALLENGE_BIG_TIME) {
    if (aSeedType == SeedType::SEED_WALLNUT ||
        aSeedType == SeedType::SEED_SUNFLOWER ||
  ...
  if (aDrawSeedInMiddle) {
    SeedPacketDrawSeed(g, x, y, theSeedType, theImitaterType, aOffsetX,
                       aOffsetY, aScale);
  }
  ...
  if (thePercentDark > 0.0f) {
    ...
    TodDrawImageCelScaledF(&aPlantG, Sexy::IMAGE_SEEDS, x, y, aPacketBackground,
                           0, aPlantG.mScaleX, aPlantG.mScaleY);
    if (aDrawSeedInMiddle) {
      SeedPacketDrawSeed(&aPlantG, x, y, theSeedType, theImitaterType, aOffsetX,
                         aOffsetY, aScale);
    }
  }
  ...
  if (theDrawCost) {
    SexyString aCostStr;
    if (gLawnApp->mBoard &&
        gLawnApp->mBoard->PlantUsesAcceleratedPricing(aSeedType)) {
      if (theUseCurrentCost) {
        aCostStr = StrFormat(_S("%d"), gLawnApp->mBoard->GetCurrentPlantCost(
                                           theSeedType, theImitaterType));
      } else {
        aCostStr =
            StrFormat(_S("%d+"), Plant::GetCost(theSeedType, theImitaterType));
      }
    } else {
      aCostStr =
          StrFormat(_S("%d"), Plant::GetCost(theSeedType, theImitaterType));
    ...
    if (g->mScaleX == 1.0f && g->mScaleY == 1.0f) {
      TodDrawString(g, aCostStr, x + aTextOffsetX, y + aTextOffsetY, aTextFont,
                    Color::Black, DS_ALIGN_LEFT);
    } else {
      SexyMatrix3 aMatrix;
      TodScaleTransformMatrix(aMatrix, aTextOffsetX * g->mScaleX + x,
                              aTextOffsetY * g->mScaleY + y, g->mScaleX,
                              g->mScaleY);
      if (g->mScaleX > 1.8f) {
...
void SeedPacket::Draw(Graphics *g) {
  ...
  if (!mActive) {
    if (mRefreshTime == 0) {
      ...
      aPercentDark =
          (float)(mRefreshTime - mRefreshCounter) / (float)mRefreshTime;
    }
  }
  ...
  if (mSlotMachineCountDown > 0) {
    ...
    DrawSeedPacket(&aClipG, 0.0f, aOffsetY, mPacketType, SeedType::SEED_NONE,
                   0.0f, 128, false, false);
    ...
    if (mPacketType == SeedType::SEED_IMITATER &&
        mImitaterType != SeedType::SEED_NONE) {
    ...
    if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED &&
      ...
      aPercentDark = 0.0f;
               ...
               mPacketType == SeedType::SEED_PEASHOOTER) {
      ...
      aGrayness = GetFlashingColor(mBoard->mMainCounter, 75).mRed;
               ...
               TutorialState::TUTORIAL_WHACK_A_ZOMBIE_PICK_SEED) {
      aGrayness = GetFlashingColor(mBoard->mMainCounter, 75).mRed;
    ...
    DrawSeedPacket(g, mOffsetX, 0.0f, mPacketType, mImitaterType, aPercentDark,
                   aGrayness, aDrawCost, true);
  }
}
...
void SeedPacket::MouseDown(int x, int y, int theClickCount) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
    ...
    if (mBoard->mTutorialState ==
        TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER) {
      mBoard->SetTutorialState(
          TutorialState::TUTORIAL_LEVEL_1_PLANT_PEASHOOTER);
    } else if (mBoard->mTutorialState ==
               TutorialState::TUTORIAL_LEVEL_2_PICK_UP_SUNFLOWER) {
      ...
      mBoard->SetTutorialState(
          TutorialState::TUTORIAL_WHACK_A_ZOMBIE_COMPLETED);
    }
...
void SeedBank::Draw(Graphics *g) {
  ...
  if (mApp->IsSlotMachineLevel()) {
    ...
    Rect theSrcRect(IMAGE_SEEDBANK->mWidth - aExtraWidth - 12, 0,
                    aExtraWidth + 12, IMAGE_SEEDBANK->mHeight);
  ...
  if (!mBoard->HasConveyorBeltSeedBank()) {
    ...
    if (mBoard->mOutOfMoneyCounter > 0 &&
        mBoard->mOutOfMoneyCounter % 20 < 10) {
