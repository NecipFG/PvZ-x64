...
void Board::PickZombieWaves()
{
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
		{
			aZombiePoints = aWave * 2 / 5 + 2;
		}
		else
		{
		...
		else if (mApp->IsMiniBossLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
		{
			aZombiePoints *= 3;
		}
...
void Board::LoadBackgroundImages()
{
	...
	case BackgroundType::BACKGROUND_WALL_KNIGHT:
		TodLoadResources("DelayLoad_BackgroundWallKnight");
		break;
...
void Board::PickBackground()
{
	...
	case GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT:
		mBackground = BackgroundType::BACKGROUND_WALL_KNIGHT;
		break;
	...
	if (mBackground == BackgroundType::BACKGROUND_1_DAY || mBackground == BackgroundType::BACKGROUND_GREENHOUSE || mBackground == BackgroundType::BACKGROUND_TREEOFWISDOM || mBackground == BackgroundType::BACKGROUND_WALL_KNIGHT)
	{
...
bool Board::ChooseSeedsOnCurrentLevel()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ICE ||
		...
		mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
	{
...
void Board::Update()
{
	...
	{
		mSpeedUpButton->mDisabled = aDisabled || mApp->mGameScene != GameScenes::SCENE_PLAYING;
	}
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_WALL_KNIGHT:		aBgImage = Sexy::IMAGE_BACKGROUNDWALLKNIGHT;			break;
...
void Board::DrawGameObjects(Graphics* g)
{
	...
	if (mApp->IsStormyNightLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
	{
		AddUIRenderItem(aRenderList, aRenderItemCount, RenderObjectType::RENDER_ITEM_STORM_BACKGROUND, MakeRenderOrder(RenderLayer::RENDER_LAYER_LAWN, 0, 0));
	...
	for (int i = 0; i < aRenderItemCount; i++)
	{
		...
		case RenderObjectType::RENDER_ITEM_PLANT:
		{
			...
			{
				aPlant->Draw(g);
				if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT && mChallenge->mWallKnight == aPlant)
				{
					DrawWallKnightHealthBar(g);
				}
				aPlant->EndDraw(g);
			}
			break;
...
void Board::DrawWallKnightHealthBar(Graphics* g)
{
	if (mChallenge->mWallKnight == nullptr || mChallenge->mWallKnight->mDead)
		return;

	int aMaxHealth = mChallenge->mWallKnight->mPlantMaxHealth;
	int aCurHealth = mChallenge->mWallKnight->mPlantHealth;

	int aWidth = 60;
	int aHeight = 10;
	int aX = (mChallenge->mWallKnight->mWidth - aWidth) / 2;
	int aY = -20;
	...
	// Health (Green/Red)
	float aHealthPerc = (float)aCurHealth / (float)aMaxHealth;
	Color aHealthColor = Color(0, 255, 0);
	if (aHealthPerc < 0.25f) aHealthColor = Color(255, 0, 0);
	else if (aHealthPerc < 0.5f) aHealthColor = Color(255, 255, 0);

	g->SetColor(aHealthColor);
	g->FillRect(aX + 2, aY + 2, (int)((aWidth - 4) * aHealthPerc), aHeight - 4);
}
	...
	case DebugTextMode::DEBUG_TEXT_MUSIC:
	{
		...
		else
		{
				...
				mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_ROOF_GRAZETHEROOF)
			{
				...
				float f1, f2, f3;
				gBass->BASS_ChannelGetAttribute(aMusicHandle1, BASS_ATTRIB_MUSIC_BPM, &f1);
				gBass->BASS_ChannelGetAttribute(aMusicHandle2, BASS_ATTRIB_MUSIC_BPM, &f2);
				gBass->BASS_ChannelGetAttribute(aMusicHandle3, BASS_ATTRIB_MUSIC_BPM, &f3);
				int bpm1 = (int)f1;
				int bpm2 = (int)f2;
				int bpm3 = (int)f3;
...
void Board::KeyDown(KeyCode theKey)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT)
	{
		mChallenge->WallKnightKeyDown(theKey);
	}
