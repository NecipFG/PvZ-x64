...
#include "Widget/HotkeyHelpDialog.h"
...
Board::Board(LawnApp *theApp) {
  ...
  mSpeedUpButton->mDrawStoneButton = true;
  mSpeedUpButton->SetLabel(_S("2x"));
  mSpeedUpButton->Resize(710, 35, 60, 30);
  mSpeedUpFlashCounter = 0;
  mAutoCollectSunButton = new GameButton(3);
  mAutoCollectSunButton->mDrawStoneButton = true;
  mAutoCollectSunButton->SetLabel(_S("AUTO"));
  mAutoCollectSunButton->Resize(645, 35, 60, 30);
  mAutoCollectSunFlashCounter = 0;
  mAutoCollectSunEnabled = false;
...
Board::~Board() {
  ...
  if (mAutoCollectSunButton) {
    delete mAutoCollectSunButton;
  }
  if (mStoreButton) {
    delete mStoreButton;
  }
...
bool Board::AreEnemyZombiesOnScreen() {
  ...
  while (IterateZombies(aZombie)) {
    if (aZombie->mHasHead && !aZombie->IsDeadOrDying() &&
...
int Board::CountZombiesOnScreen() {
  ...
  while (IterateZombies(aZombie)) {
    if (aZombie->mHasHead && !aZombie->IsDeadOrDying() &&
        !aZombie->mMindControlled && aZombie->IsOnBoard()) {
      aCount++;
    }
  }
  return aCount;
...
void Board::TryToSaveGame() {
  std::string aFileName =
      GetSavedGameName(mApp->mGameMode, mApp->mPlayerInfo->mId);
...
// 0x408E40
GridItem *Board::GetGridItemAt(GridItemType theGridItemType, int theGridX,
                               int theGridY) {
...
bool Board::CanAddGraveStoneAt(int theGridX, int theGridY) {
  if (mGridSquareType[theGridX][theGridY] != GridSquareType::GRIDSQUARE_GRASS &&
      mGridSquareType[theGridX][theGridY] !=
...
int Board::MakeRenderOrder(RenderLayer theRenderLayer, int theRow,
                           int theLayerOffset) {
  return theRow * (int)RenderLayer::RENDER_LAYER_ROW_OFFSET + theRenderLayer +
         theLayerOffset;
}
...
GridItem *Board::AddALadder(int theGridX, int theGridY) {
  ...
  aLadder->mRenderOrder =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, theGridY, 800);
...
GridItem *Board::AddACrater(int theGridX, int theGridY) {
  ...
  aCrater->mRenderOrder =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, theGridY, 1);
...
GridItem *Board::AddAGraveStone(int theGridX, int theGridY) {
  ...
  aGraveStone->mRenderOrder =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, theGridY, 3);
...
void Board::AddGraveStones(int theGridX, int theCount, MTRand &theLevelRNG) {
  ...
  while (i < theCount) {
    ...
    //	++i;
    // }
    //  上述写法虽然效率更高，但当 AddAGraveStone() 函数被修改后，不能保证
    //  aAllowGraveStone 仍然有效 故这里仍然采用如下的原版的写法，仅在上面对
    //  theCount 进行修正
...
void Board::PutZombieInWave(ZombieType theZombieType, int theWaveNumber,
  ...
  mZombiesInWave[theWaveNumber][theZombiePicker->mZombieCount++] =
      theZombieType;
  if (theZombiePicker->mZombieCount < MAX_ZOMBIES_IN_WAVE) {
    mZombiesInWave[theWaveNumber][theZombiePicker->mZombieCount] =
        ZombieType::ZOMBIE_INVALID;
  }
  theZombiePicker->mZombiePoints -=
      GetZombieDefinition(theZombieType).mZombieValue;
  theZombiePicker->mZombieTypeCount[theZombieType]++;
  theZombiePicker->mAllWavesZombieTypeCount[theZombieType]++;
}

// 0x409290
void Board::PutInMissingZombies(int theWaveNumber,
                                ZombiePicker *theZombiePicker) {
...
void Board::PickZombieWaves() {
  ...
  for (int aWave = 0; aWave < mNumWaves; aWave++) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND) {
      aZombiePoints =
          (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave +
           10) *
              2 / 5 +
          1;
      ...
      aZombiePoints =
          (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave) *
              2 / 5 +
          1;
    } else if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() &&
               mLevel != 5) {
    ...
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
      ...
      if (aWave % 10 == 8) {
        for (int _i = 0; _i < 10; _i++) {
          PutZombieInWave(ZombieType::ZOMBIE_JACK_IN_THE_BOX, aWave,
                          &aZombiePicker);
        }
      }
    ...
    // △ 剩余的僵尸点数用于向列表中补充随机僵尸
    // ------------------------------------------------------------------------------------------------
    while (aZombiePoints > 0 &&
           aZombiePicker.mZombieCount < MAX_ZOMBIES_IN_WAVE) {
      ZombieType aZombieType =
          PickZombieType(aZombiePoints, aWave, &aZombiePicker);
...
void Board::LoadBackgroundImages() {
  ...
  case BackgroundType::BACKGROUND_TREEOFWISDOM:
    ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_TREEOFWISDOM,
                                     true);
    break;
...
void Board::PickBackground() {
  ...
  for (int x = 0; x < MAX_GRID_SIZE_X; x++) {
    for (int y = 0; y < MAX_GRID_SIZE_Y; y++) {
      ...
      } else if (mPlantRow[y] == PlantRowType::PLANTROW_POOL && x >= 0 &&
                 x <= 8) {
        mGridSquareType[x][y] = GridSquareType::GRIDSQUARE_POOL;
      } else if (mPlantRow[y] == PlantRowType::PLANTROW_HIGH_GROUND && x >= 4 &&
                 x <= 8) {
...
void Board::InitZombieWavesForLevel(int theForLevel) {
  ...
  for (int aZombieType = ZombieType::ZOMBIE_NORMAL;
       aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType++) {
    mZombieAllowed[aZombieType] =
        CanZombieSpawnOnLevel((ZombieType)aZombieType, theForLevel);
  }
}
...
bool Board::IsZombieWaveDistributionOk() {
  ...
  for (ZombieType aZombieType = ZombieType::ZOMBIE_NORMAL;
       aZombieType < ZombieType::NUM_ZOMBIE_TYPES;
       aZombieType = (ZombieType)((int)aZombieType + 1)) {
    if (aZombieType != ZombieType::ZOMBIE_YETI &&
        CanZombieSpawnOnLevel(aZombieType, mLevel) &&
...
Rect Board::GetShovelButtonRect() {
  Rect aRect(GetSeedBankExtraWidth() + 456, 0,
             Sexy::IMAGE_SHOVELBANK->GetWidth(),
...
void Board::GetZenButtonRect(GameObjectType theObjectType, Rect &theRect) {
  ...
  for (int anObject = GameObjectType::OBJECT_TYPE_WATERING_CAN;
       anObject < theObjectType; anObject++) {
...
void Board::InitLevel() {
  ...
  if (mApp->IsSlotMachineLevel()) {
    ...
    mSeedBank->mSeedPackets[2].SetPacketType(SeedType::SEED_TORCHWOOD);
    mSeedBank->mSeedPackets[3].SetPacketType(SeedType::SEED_SNOWPEA);
    mSeedBank->mSeedPackets[4].SetPacketType(SeedType::SEED_WALLNUT);
    ...
    mSeedBank->mSeedPackets[0].SetPacketType(
        SeedType::SEED_ZOMBIQUARIUM_SNORKLE);
    mSeedBank->mSeedPackets[1].SetPacketType(
        SeedType::SEED_ZOMBIQUARIUM_TROPHY);
    ...
    mSeedBank->mSeedPackets[2].SetPacketType(mApp->IsAdventureMode()
...
Reanimation *Board::CreateRakeReanim(float theRakeX, float theRakeY,
                                     int theRenderOrder) {
  Reanimation *aReanim = mApp->AddReanimation(
      GridToPixelX(theRakeX, theRakeY) + 20, GridToPixelY(theRakeX, theRakeY),
...
void Board::PlaceRake() {
  ...
  aRake->mRenderOrder =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, aGridY, 9);
  aRake->mGridItemReanimID =
      mApp->ReanimationGetID(CreateRakeReanim(aGridX, aGridY, 0));
...
void Board::InitLawnMowers() {
  ...
  if (aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
      ...
      (StageHasRoof() &&
       !mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_ROOF_CLEANER]))
    return;

  for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
    if ((aGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED && aRow <= 4) ||
        (mApp->IsAdventureMode() &&
         mLevel == 35) || // 这里原版没有对于行的判断，故冒险模式
                          // 4-5 关卡有 6 行小推车
        (!mApp->IsScaryPotterLevel() &&
         mPlantRow[aRow] !=
             PlantRowType::PLANTROW_DIRT)) // 除冒险模式 4-5
                                           // 关卡外的破罐者模式关卡无小推车
    {
...
void Board::UpdateLevelEndSequence() {
  ...
  if (mBoardFadeOutCounter == 300) {
    if (!IsSurvivalStageWithRepick() &&
        !(mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 ||
          mLevel == 49)) {
  ...
  if (CanDropLoot() && !IsSurvivalStageWithRepick()) {
    ...
    if (aLawnMower) {
      AddCoin(aLawnMower->mPosX + 40, aLawnMower->mPosY + 40,
              CoinType::COIN_GOLD, CoinMotion::COIN_MOTION_LAWNMOWER_COIN);
...
void Board::FadeOutLevel() {
  ...
  if (mApp->IsScaryPotterLevel() && !IsFinalScaryPotterStage()) {
    ...
    if (mApp->IsAdventureMode()) {
      ...
      SexyString aStreakStr = mApp->IsEndlessScaryPotter(mApp->mGameMode)
                                  ? _S("[ADVICE_MORE_SCARY_POTS]")
                                  : _S("[ADVICE_3_IN_A_ROW]");
      SexyString aMessage = TodReplaceNumberString(
          aStreakStr, _S("{STREAK}"), mChallenge->mSurvivalStage + 1);
      PuzzleSaveStreak();
      ClearAdvice(AdviceType::ADVICE_NONE);
  ...
  if (mApp->IsEndlessIZombie(mApp->mGameMode)) {
    mNextSurvivalStageCounter = 500;
    SexyString aMessage =
        TodReplaceNumberString(_S("[ADVICE_MORE_IZOMBIE]"), _S("{STREAK}"),
  ...
  if (!IsSurvivalStageWithRepick()) {
    ...
    if (mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 ||
        mLevel == 49) {
    ...
    mNextSurvivalStageCounter = 500;
    DisplayAdvice(_S("[ADVICE_MORE_ZOMBIES]"),
...
void Board::DisplayAdvice(const SexyString &theAdvice,
...
// 0x40CA10
void Board::DisplayAdviceAgain(const SexyString &theAdvice,
                               MessageStyle theMessageStyle,
                               AdviceType theHelpIndex) {
  if (theHelpIndex != AdviceType::ADVICE_NONE) {
...
// 0x40CB10
Coin *Board::AddCoin(int theX, int theY, CoinType theCoinType,
...
bool Board::IsPlantInCursor() {
  return mCursorObject->mCursorType ==
             CursorType::CURSOR_TYPE_PLANT_FROM_BANK ||
         mCursorObject->mCursorType ==
...
SeedType Board::GetSeedTypeInCursor() {
  ...
  return mCursorObject->mType == SeedType::SEED_IMITATER
             ? mCursorObject->mImitaterType
             : mCursorObject->mType;
}
...
void Board::RefreshSeedPacketFromCursor() {
  ...
  } else if (mCursorObject->mCursorType ==
             CursorType::CURSOR_TYPE_PLANT_FROM_BANK) {
...
bool Board::IsPoolSquare(int theGridX, int theGridY) {
  if (theGridX >= 0 && theGridY >= 0) {
    ...
    return mGridSquareType[theGridX][theGridY] ==
           GridSquareType::GRIDSQUARE_POOL;
  }
  return false;
}

// 0x40CE20
Plant *Board::NewPlant(int theGridX, int theGridY, SeedType theSeedType,
...
// 0x40D120
Plant *Board::AddPlant(int theGridX, int theGridY, SeedType theSeedType,
                       SeedType theImitaterType) {
  ...
  int aSunPlantsCount = CountPlantByType(SeedType::SEED_SUNSHROOM) +
                        CountPlantByType(SeedType::SEED_SUNFLOWER);
  if (aSunPlantsCount > mMaxSunPlants) {
    mMaxSunPlants =
        aSunPlantsCount; // mMaxSunPlants = max(aSunPlantsCount, mMaxSunPlants);
  }
...
// 0x40D2A0
void Board::GetPlantsOnLawn(int theGridX, int theGridY,
                            PlantsOnLawn *thePlantOnLawn) {
  ...
  while (IteratePlants(aPlant)) {
    ...
    if (aSeedType == SeedType::SEED_IMITATER &&
        aPlant->mImitaterType != SeedType::SEED_NONE) {
...
Plant *Board::GetTopPlantAt(int theGridX, int theGridY,
                            PlantPriority thePriority) {
  if (theGridX < 0 || theGridX >= MAX_GRID_SIZE_X || theGridY < 0 ||
      theGridY >= MAX_GRID_SIZE_Y)
    return nullptr;
...
bool Board::IsValidCobCannonSpotHelper(int theGridX, int theGridY) {
  ...
  return mApp->mEasyPlantingCheat &&
         CanPlantAt(theGridX, theGridY, SeedType::SEED_KERNELPULT) ==
...
bool Board::IsValidCobCannonSpot(int theGridX, int theGridY) {
  ...
  return !GetFlowerPotAt(theGridX, theGridY) ==
         !GetFlowerPotAt(theGridX + 1, theGridY);
}
...
// 0x40D620
Projectile *Board::AddProjectile(int theX, int theY, int theRenderOrder,
                                 int theRow, ProjectileType theProjectileType) {
...
bool Board::CanZombieSpawnOnLevel(ZombieType theZombieType, int theLevel) {
  ...
  return gZombieAllowedLevels[theZombieType]
      .mAllowedOnLevel[ClampInt(theLevel - 1, 0, FINAL_LEVEL - 1)];
}
...
ZombieType Board::PickGraveRisingZombieType(int theZombiePoints) {
  ...
  aZombieWeightArray[0].mWeight =
      GetZombieDefinition(ZombieType::ZOMBIE_NORMAL).mPickWeight;
  ...
  if (!StageHasGraveStones()) {
    ...
    aZombieWeightArray[2].mWeight =
        GetZombieDefinition(ZombieType::ZOMBIE_PAIL).mPickWeight;
    aCount++;
  }
  ...
  for (int i = 0; i < aCount; i++) {
    ...
    if ((mApp->IsFirstTimeAdventureMode() &&
         mLevel < aZombieDef.mStartingLevel) ||
        (!mZombieAllowed[aZombieType] &&
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex,
  ...
  for (int aZombieType = ZombieType::ZOMBIE_NORMAL;
    ...
    const ZombieDefinition &aZombieDef =
        GetZombieDefinition((ZombieType)aZombieType);
    ...
    if (aZombieType == ZombieType::ZOMBIE_BUNGEE &&
        mApp->IsSurvivalEndless(aGameMode)) {
    ...
    // 僵尸最早出现的波数的限制（出怪限制）
      ...
      if (theWaveIndex + 1 < aFirstAllowedWave ||
          theZombiePoints < aZombieDef.mZombieValue) {
        continue;
      }
    }
    ...
    if (mApp->IsSurvivalMode()) {
      ...
      // 普通僵尸和路障僵尸的权重衰减
        ...
        aPickWeight =
            TodAnimateCurve(10, 50, aFlags, aPickWeight, aPickWeight / 10,
                            TodCurves::CURVE_LINEAR);
      } else if (aZombieType == ZombieType::ZOMBIE_TRAFFIC_CONE) {
        aPickWeight = TodAnimateCurve(10, 50, aFlags, aPickWeight,
                                      aPickWeight / 4, TodCurves::CURVE_LINEAR);
      }
    }
...
int Board::PickRowForNewZombie(ZombieType theZombieType) {
  ...
  for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
    ...
    // 保护传送门关卡中，每行的出怪概率受传送门位置影响
      ...
      mRowPickingArray[aRow].mWeight =
          mChallenge->PortalCombatRowSpawnWeight(aRow);
    }
...
// 0x40DDC0
Zombie *Board::AddZombieInRow(ZombieType theZombieType, int theRow,
                              int theFromWave) {
  ...
  aZombie->ZombieInitialize(theRow, theZombieType, aVariant, nullptr,
                            theFromWave);
...
Zombie *Board::AddZombie(ZombieType theZombieType, int theFromWave) {
  return AddZombieInRow(theZombieType, PickRowForNewZombie(theZombieType),
                        theFromWave);
}
...
// 0x40E020
PlantingReason Board::CanPlantAt(int theGridX, int theGridY,
  ...
  // 从关卡玩法上，判断能否种植
  PlantingReason aReason =
      mChallenge->CanPlantAt(theGridX, theGridY, theSeedType);
  if (aReason != PlantingReason::PLANTING_OK ||
      Challenge::IsZombieSeedType(theSeedType)) {
    return aReason;
  }
  ...
  GetPlantsOnLawn(theGridX, theGridY, &aPlantOnLawn);
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
    if (aPlantOnLawn.mUnderPlant || aPlantOnLawn.mPumpkinPlant ||
        aPlantOnLawn.mFlyingPlant || aPlantOnLawn.mNormalPlant) {
      return PlantingReason::PLANTING_NOT_HERE;
    }
    if (mApp->mZenGarden->mGardenType == GARDEN_AQUARIUM &&
        !Plant::IsAquatic(theSeedType)) {
  ...
  if (theSeedType == SeedType::SEED_GRAVEBUSTER) {
    ...
    return aHasGrave ? PlantingReason::PLANTING_OK
                     : PlantingReason::PLANTING_ONLY_ON_GRAVES;
  }
  ...
  if (!aUnderPlant || aUnderPlant->mOnBungeeState ==
                          PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE) {
    aHasLilypad = false;
    aHasFlowerPot = false;
  } else {
  ...
  if (theSeedType == SeedType::SEED_SPIKEWEED ||
    ...
    if (aGridSquare == GridSquareType::GRIDSQUARE_POOL || StageHasRoof() ||
        aUnderPlant) {
  ...
  if (theSeedType == SeedType::SEED_FLOWERPOT) {
    return (aNormalPlant || aUnderPlant || aPumpkinPlant)
               ? PlantingReason::PLANTING_NOT_HERE
               : PlantingReason::PLANTING_OK;
  }
  // 屋顶种植需要花盆
  ...
  if (theSeedType == SeedType::SEED_PUMPKINSHELL) {
    ...
    // 南瓜头的坚果包扎术
    if (aAidPurchased &&
        aPumpkinPlant->mPlantHealth < aPumpkinPlant->mPlantMaxHealth * 2 / 3 &&
        aPumpkinPlant->mSeedType == SeedType::SEED_PUMPKINSHELL &&
        aPumpkinPlant->mOnBungeeState !=
            PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE) {
      return PlantingReason::PLANTING_OK;
    }
  ...
  // 免费种植模式下紫卡的额外判断
  if (!mApp->mEasyPlantingCheat && Plant::IsUpgrade(theSeedType) && mLevel != 60) {
    return PlantingReason::PLANTING_NEEDS_UPGRADE;
  }
  if (theSeedType == SeedType::SEED_COBCANNON &&
      !IsValidCobCannonSpot(theGridX, theGridY)) {
    return PlantingReason::PLANTING_NEEDS_UPGRADE;
...
void Board::UpdateCursor() {
  ...
  if (mApp->mSeedChooserScreen &&
      mApp->mSeedChooserScreen->Contains(aMouseX + mX, aMouseY + mY))
    return;
  ...
  case GameObjectType::OBJECT_TYPE_PLANT:
    ...
    if (((Plant *)aHitResult.mObject)->mState ==
        PlantState::STATE_COBCANNON_READY) {
      aShowFinger = true;
    }
    break;
...
// 0x40E880
bool Board::IsPlantInGoldWateringCanRange(int theMouseX, int theMouseY,
                                          Plant *thePlant) {
  /*
...
void Board::HighlightPlantsForMouse(int theMouseX, int theMouseY) {
  if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_WATERING_CAN &&
      mApp->mPlayerInfo
          ->mPurchases[(int)StoreItem::STORE_ITEM_GOLD_WATERINGCAN]) {
    Plant *aPlant = nullptr;
    while (IteratePlants(aPlant)) {
...
void Board::UpdateMousePosition() {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
    ...
    if (aStinky) {
      HitResult aHitResult;
      MouseHitTest(aMouseX, aMouseY, &aHitResult);
      aStinky->mHighlighted =
          aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_STINKY;
    }
  }
  ...
  if (aCursorSeedType == SeedType::SEED_INSTANT_COFFEE) {
    ...
    Plant *aPlant = GetTopPlantAt(aGridX, aGridY,
                                  PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
    ...
    Plant *aPlant =
        GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_PUMPKIN);
    if (aPlant && aPlant->mSeedType == aCursorSeedType &&
        CanPlantAt(aGridX, aGridY, aCursorSeedType) ==
            PlantingReason::PLANTING_OK) {
      aPlant->mHighlighted = true;
    }
...
void Board::UpdateToolTip() {
  if (!mApp->mWidgetManager->mMouseIn || !mApp->mActive ||
      mTimeStopCounter > 0 || mApp->GetDialogCount() > 0 ||
      mApp->mGameScene == GameScenes::SCENE_ZOMBIES_WON) {
  ...
  if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO) {
    ...
    if (mSeedBank->ContainsPoint(mWidgetManager->mLastMouseX,
    ...
    if (aZombie == nullptr ||
        aZombie->mFromWave != Zombie::ZOMBIE_WAVE_CUTSCENE) {
      mToolTip->mVisible = false;
      return;
    }

    SexyString aZombieName = StrFormat(
        _S("[%s]"), GetZombieDefinition(aZombie->mZombieType).mZombieName);
  ...
  if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_WATERING_CAN ||
    ...
    if (aHitResult.mObjectType == GameObjectType::OBJECT_TYPE_WATERING_CAN) {
    ...
    } else if (aHitResult.mObjectType ==
               GameObjectType::OBJECT_TYPE_FERTILIZER) {
      mToolTip->SetLabel(_S("[FERTILIZER_TOOLTIP]"));
    } else if (aHitResult.mObjectType ==
               GameObjectType::OBJECT_TYPE_BUG_SPRAY) {
      mToolTip->SetLabel(_S("[BUG_SPRAY_TOOLTIP]"));
    } else if (aHitResult.mObjectType ==
               GameObjectType::OBJECT_TYPE_PHONOGRAPH) {
      mToolTip->SetLabel(_S("[PHONOGRAPH_TOOLTIP]"));
    } else if (aHitResult.mObjectType ==
               GameObjectType::OBJECT_TYPE_CHOCOLATE) {
      ...
      mToolTip->SetLabel(_S("[GLOVE_TOOLTIP]"));
               ...
               GameObjectType::OBJECT_TYPE_MONEY_SIGN) {
      mToolTip->SetLabel(_S("[MONEY_SIGN_TOOLTIP]"));
    } else if (aHitResult.mObjectType ==
               GameObjectType::OBJECT_TYPE_WHEELBARROW) {
  ...
  if (gLawnApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
    ...
    mToolTip->SetLabel(
        StrFormat(_S("[%s]"), GetPlantDefinition(aUseSeedType).mPlantName));
  }

  int aPlantCost =
      GetCurrentPlantCost(aSeedPacket->mPacketType, aSeedPacket->mImitaterType);
...
void Board::MouseDownCobcannonFire(int x, int y, int theClickCount) {
  if (theClickCount >= 0 && y >= 80) {
    if (mCobCannonCursorDelayCounter > 0 &&
      ...
      return; // 误点检测：点击加农炮后的 30cs
              // 内，点击的位置和准心位置之间的距离小于 100 时，将被判定为误点
    }
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount) {
  ...
  // 不在场地内的点击：放下卡牌
  if (aGridX < 0 || aGridX >= MAX_GRID_SIZE_X || aGridY < 0 ||
      aGridY > MAX_GRID_SIZE_Y) {
    RefreshSeedPacketFromCursor();
    mApp->PlayFoley(FoleyType::FOLEY_DROP);
    return;
  }
  ...
  if (aReason != PlantingReason::PLANTING_OK) {
    ...
    if (aReason == PlantingReason::PLANTING_ONLY_ON_GRAVES) {
      ...
      SexyString aMessage = TodReplaceString(_S("[ADVICE_WRONG_ART_TYPE]"),
                                             _S("{SEED}"), aSeedName);
      ...
      if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
        ...
        DisplayAdvice(_S("[ZEN_ONLY_AQUATIC_PLANTS]"),
                      MessageStyle::MESSAGE_STYLE_HINT_FAST,
                      AdviceType::ADVICE_NONE);
  ...
  if (!mApp->mEasyPlantingCheat &&
    ...
    if (!TakeSunMoney(
            GetCurrentPlantCost(aPlantingSeedType, SeedType::SEED_NONE))) {
      return;
    }
  }
  ...
  if (aPlantingSeedType == SeedType::SEED_COBCANNON) {
    Plant *aRightPlant = GetTopPlantAt(
        aGridX + 1, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
    if (aRightPlant) {
      aRightPlant->Die();
    }
  }
  ...
  if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_GLOVE) {
    mApp->mZenGarden->MovePlant(
        mPlants.DataArrayTryToGet(mCursorObject->mGlovePlantID), aGridX,
        aGridY);
  } else if (mCursorObject->mCursorType ==
             CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW) {
    mApp->mZenGarden->MouseDownWithFullWheelBarrow(x, y);
             ...
             CursorType::CURSOR_TYPE_PLANT_FROM_USABLE_COIN) {
    AddPlant(aGridX, aGridY, mCursorObject->mType,
             mCursorObject->mImitaterType);
    ...
    aCoin->Die();
             ...
             CursorType::CURSOR_TYPE_PLANT_FROM_BANK) {
    Plant *aPlant = AddPlant(aGridX, aGridY, mCursorObject->mType,
                             mCursorObject->mImitaterType);
    if (aIsAwake) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN) {
    for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
      if (aRow == aGridY || CanPlantAt(aGridX, aRow, aPlantingSeedType) !=
                                PlantingReason::PLANTING_OK)
        continue;

      if (aPlantingSeedType == SeedType::SEED_WALLNUT ||
          aPlantingSeedType == SeedType::SEED_TALLNUT) {
        aNormalPlant = GetTopPlantAt(
            aGridX, aRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
      ...
      if (aPlantingSeedType == SeedType::SEED_PUMPKINSHELL) {
        aPumpkinPlant =
            GetTopPlantAt(aGridX, aRow, PlantPriority::TOPPLANT_ONLY_PUMPKIN);
        if (aPumpkinPlant &&
            aPumpkinPlant->mSeedType == SeedType::SEED_PUMPKINSHELL) {
          aPumpkinPlant->Die();
        }
      }
      AddPlant(aGridX, aRow, mCursorObject->mType,
               mCursorObject->mImitaterType);
    }
  }
...
void Board::TutorialArrowShow(int theX, int theY) {
  ...
  TodParticleSystem *aParticle = mApp->AddTodParticle(
      theX, theY, MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
...
// 0x411060
void Board::MouseDownWithTool(int x, int y, int theClickCount,
                              CursorType theCursorType) {
  if (theClickCount < 0) {
    ClearCursor();
    mApp->PlayFoley(FoleyType::FOLEY_DROP);
    return;
  ...
  if (aPlant == nullptr) {
    ...
    if (aPlant->mSeedType == SeedType::SEED_CATTAIL &&
        GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow,
                      PlantPriority::TOPPLANT_ONLY_PUMPKIN)) {
      NewPlant(aPlant->mPlantCol, aPlant->mRow, SeedType::SEED_LILYPAD,
               SeedType::SEED_NONE);
    }
...
bool Board::MouseHitTestPlant(int x, int y, HitResult *theHitResult) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
    aPlant =
        GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ZEN_TOOL_ORDER);
    if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_WATERING_CAN &&
        (!aPlant || !mApp->mZenGarden->PlantCanBeWatered(aPlant))) {
      Plant *aTopPlant = GetTopPlantAt(PixelToGridX(x - 30, y - 20),
                                       PixelToGridY(x - 30, y - 20),
    ...
    aPlant =
        GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_DIGGING_ORDER);
...
bool Board::MouseHitTest(int x, int y, HitResult *theHitResult) {
  ...
  if (mShowShovel && aShovelButtonRect.Contains(x, y) &&
      CanInteractWithBoardButtons()) {
  ...
  if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_NORMAL ||
    ...
    while (IterateCoins(aCoin)) {
      ...
      if (aCoin->MouseHitTest(x, y, &aHitResultCoin)) {
        ...
        if (aTopCoin == nullptr ||
            aCoin->mRenderOrder >= aTopCoin->mRenderOrder) {
  ...
  if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
    ...
    for (int i = (int)GameObjectType::OBJECT_TYPE_WATERING_CAN;
      ...
      if (CanUseGameObject(aTool) &&
          (aTool != GameObjectType::OBJECT_TYPE_TREE_FOOD ||
  ...
  if (mApp->IsScaryPotterLevel() &&
      ...
      mChallenge->mChallengeState !=
          ChallengeState::STATECHALLENGE_SCARY_POTTER_MALLETING &&
      mApp->mGameScene == GameScenes::SCENE_PLAYING &&
      mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) == nullptr) {
    GridItem *aScaryPot = GetGridItemAt(GridItemType::GRIDITEM_SCARY_POT,
                                        PixelToGridX(x, y), PixelToGridY(x, y));
    if (aScaryPot) {
  ...
  if (mApp->IsSlotMachineLevel()) {
    ...
    if (aSlotMachineHandleRect.Contains(x, y) &&
        ...
        !HasLevelAwardDropped()) {
      theHitResult->mObjectType =
          GameObjectType::OBJECT_TYPE_SLOT_MACHINE_HANDLE;
      return true;
    }
  }
...
void Board::MouseDown(int x, int y, int theClickCount) {
  ...
  if (mMenuButton->IsMouseOver() && CanInteractWithBoardButtons() &&
    ...
    mApp->PlaySample(Sexy::SOUND_GRAVEBUTTON);
             ...
             CanInteractWithBoardButtons() && theClickCount > 0) {
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
        mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM) {
  ...
  if (mApp->mTodCheatKeys && !mApp->IsScaryPotterLevel() &&
      mNextSurvivalStageCounter > 0) {
  ...
  if (aCursor == CursorType::CURSOR_TYPE_SHOVEL ||
    ...
    if (!mPaused) {
      ((SeedPacket *)aHitResult.mObject)->MouseDown(x, y, theClickCount);
             ...
             GameObjectType::OBJECT_TYPE_NEXT_GARDEN) {
    if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
...
void Board::MouseUp(int x, int y, int theClickCount) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED &&
      mChallenge->MouseUp(x, y) && theClickCount > 0)
    return;
  ...
  if (CanInteractWithBoardButtons() && theClickCount > 0) {
    ...
    } else if (mSpeedUpButton->IsMouseOver() && !mSpeedUpButton->mDisabled) {
      if (mApp->mUpdateMultiplier == 1.0) {
        mApp->mUpdateMultiplier = 2.0;
      } else {
        mApp->mUpdateMultiplier = 1.0;
      }
      mSpeedUpFlashCounter = 100;
      mApp->PlaySample(Sexy::SOUND_BUTTONCLICK);
    } else if (mAutoCollectSunButton->IsMouseOver() && !mAutoCollectSunButton->mDisabled) {
      mAutoCollectSunEnabled = !mAutoCollectSunEnabled;
      mAutoCollectSunButton->SetLabel(mAutoCollectSunEnabled ? _S("AUTO*") : _S("AUTO"));
      mAutoCollectSunFlashCounter = 100;
      mApp->PlaySample(Sexy::SOUND_BUTTONCLICK);
    } else if (mStoreButton && mStoreButton->IsMouseOver()) {
      if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
...
void Board::PickSpecialGraveStone() {
  ...
  if (aPickCount > 0) {
    aPicks[Sexy::Rand(aPickCount)]->mGridItemState =
...
void Board::SpawnZombiesFromPool() {
  ...
  if (mLevel == 21 || mLevel == 22 || mLevel == 31 || mLevel == 32) {
    aCount = 2;
    aZombiePoints = 3;
  } else if (mLevel == 23 || mLevel == 24 || mLevel == 25 || mLevel == 33 ||
             mLevel == 34 || mLevel == 35) {
    aCount = 3;
    aZombiePoints = 5;
  } else {
  ...
  for (int i = 0; i < aCount; i++) {
    TodWeightedGridArray *aGrid =
        TodPickFromWeightedGridArray(aGridArray, aGridArrayCount);
    aGrid->mWeight = 0;
...
void Board::SetupBungeeDrop(BungeeDropGrid *theBungeeDropGrid) {
  ...
  for (int aGridX = 4; aGridX < MAX_GRID_SIZE_X; aGridX++) {
    for (int aGridY = 0; aGridY <= 4; aGridY++) {
      ...
      TOD_ASSERT(theBungeeDropGrid->mGridArrayCount <=
                 LENGTH(theBungeeDropGrid->mGridArray));
    }
  }
}

// 0x412B60
void Board::BungeeDropZombie(BungeeDropGrid *theBungeeDropGrid,
                             ZombieType theZombieType) {
  TodWeightedGridArray *aGrid = TodPickFromWeightedGridArray(
      theBungeeDropGrid->mGridArray, theBungeeDropGrid->mGridArrayCount);
  aGrid->mWeight = 1;
    ...
    for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++) {
      ...
      if (aZombieType == ZombieType::ZOMBIE_BOBSLED && !CanAddBobSled()) {
        for (int i = 0; i < MAX_ZOMBIE_FOLLOWERS; i++) {
          AddZombie(ZombieType::ZOMBIE_NORMAL,
                    mCurrentWave); // 生成 4 只普通僵尸以代替雪橇僵尸小队
        }
      } else {
        AddZombie(aZombieType, mCurrentWave);
      }
...
int Board::GetSurvivalFlagsCompleted() {
  int aWavesPerFlag = GetNumWavesPerFlag();
  int aFlagsCompleted = mChallenge->mSurvivalStage *
                        GetNumWavesPerSurvivalStage() / aWavesPerFlag;
  int aCurrentWave = mCurrentWave;
  if (IsFlagWave(aCurrentWave - 1) && mBoardFadeOutCounter < 0 &&
      !mNextSurvivalStageCounter) {
    aCurrentWave -= 1;
  }
...
void Board::ZombiesWon(Zombie *theZombie) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM) {
    ...
    SexyString aFlagStr = mApp->Pluralize(
        GetSurvivalFlagsCompleted(), _S("[ONE_FLAG]"), _S("[COUNT_FLAGS]"));
    aGameOverMsg = TodReplaceString(_S("[LAST_STAND_DEATH_MESSAGE]"),
                                    _S("{FLAGS}"), aFlagStr);
  } else if (mApp->IsEndlessIZombie(mApp->mGameMode) ||
             mApp->IsEndlessScaryPotter(mApp->mGameMode)) {
    aGameOverMsg =
        TodReplaceNumberString(_S("[ENDLESS_PUZZLE_DEATH_MESSAGE]"),
                               _S("{STREAK}"), mChallenge->mSurvivalStage);
  } else if (mApp->IsIZombieLevel()) {
    aGameOverMsg = _S("[I_ZOMBIE_DEATH_MESSAGE]");
  } else {
    mApp->mGameScene = GameScenes::SCENE_ZOMBIES_WON;
    if (theZombie) // 原版此处没有对 theZombie
                   // 进行空指针判断，但加上判断后便允许绕过僵尸而直接调用游戏失败
    {
...
bool Board::IsFinalSurvivalStage() {
  ...
  int aFlags = GetNumWavesPerSurvivalStage() *
               (mChallenge->mSurvivalStage + 1) / GetNumWavesPerFlag();
  if (mApp->IsSurvivalNormal(mApp->mGameMode)) {
    return aFlags >= 5;
  }
...
bool Board::IsLastStandStageWithRepick() {
  return mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND &&
         !IsLastStandFinalStage();
}

// 0x413A40
bool Board::HasLevelAwardDropped() {
  return mLevelAwardSpawned || mNextSurvivalStageCounter > 0 ||
         mBoardFadeOutCounter >= 0;
}
...
void Board::UpdateSunSpawning() {
  if (StageIsNight() || HasLevelAwardDropped() ||
      ...
      mApp->IsIZombieLevel() || mApp->IsScaryPotterLevel() ||
      mApp->IsSquirrelLevel() || HasConveyorBeltSeedBank() ||
...
void Board::NextWaveComing() {
  if (mCurrentWave + 1 == mNumWaves) {
    if (!IsSurvivalStageWithRepick() &&
      ...
      mApp->AddReanimation(
          0, 30, MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 0),
...
void Board::UpdateZombieSpawning() {
  ...
  if (mHugeWaveCountDown > 0) {
    ...
    if (mHugeWaveCountDown == 0) {
      ...
      if (mHugeWaveCountDown == 725) {
        ...
        if (mApp->mMusic->mCurMusicTune ==
                MusicTune::MUSIC_TUNE_DAY_GRASSWALK ||
            mApp->mMusic->mCurMusicTune ==
        ...
        } else if (mApp->mMusic->mCurMusicTune ==
  ...
  if (mZombieCountDown > 200 &&
      mZombieCountDownStart - mZombieCountDown > 400 &&
  ...
  if (mZombieCountDown == 5) {
    if (IsFlagWave(mCurrentWave)) {
      ClearAdviceImmediately();
      DisplayAdviceAgain(_S("[ADVICE_HUGE_WAVE]"),
  ...
  if (mZombieCountDown == 0) {
    ...
    if (mCurrentWave == mNumWaves && mApp->IsSurvivalMode()) {
      ...
      mZombieHealthToNextWave =
          RandRangeFloat(0.5f, 0.65f) * mZombieHealthWaveStart;
...
void Board::UpdateIce() {
  for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
    if (mIceTimer[aRow]) {
      mIceTimer[aRow]--;
      TodParticleSystem *aParticleIce =
          mApp->ParticleTryToGet(mIceParticleID[aRow]);
      if (mIceTimer[aRow] == 0) {
        mIceMinX[aRow] = BOARD_ICE_START;
        if (aParticleIce) {
        ...
        if (aParticleIce) {
          ...
          aParticleIce =
              mApp->AddTodParticle(aPosX, aPosY, aRenderPosition,
...
void Board::UpdateProgressMeter() {
  if (mApp->IsFinalBossLevel()) {
    ...
    if (aBoss && !aBoss->IsDeadOrDying()) {
      mProgressMeterWidth = 150 * (aBoss->mBodyMaxHealth - aBoss->mBodyHealth) /
                            aBoss->mBodyMaxHealth;
    } else {
    ...
    int aNumWavesPerFlag =
        GetNumWavesPerFlag(); // 本关卡中每相邻两个旗帜波之前的小波数量
    bool aHasFlags =
        ProgressMeterHasFlags(); // 进度条标注旗帜时，旗帜波占用更长的进度条
    if (aHasFlags) {
      aTotalWidth -=
          12 * mNumWaves / aNumWavesPerFlag; // 从每个旗帜波分割出 12 单位的长度
    }

    int aWaveLength = aTotalWidth / (mNumWaves - 1); // 每一小波占用的进度条长度
    int aCurrentWaveLength = (mCurrentWave - 1) * aTotalWidth /
                             (mNumWaves - 1); // 当前波开始时的进度条长度
    int aNextWaveLength = mCurrentWave * aTotalWidth /
                          (mNumWaves - 1); // 下一波开始时的进度条长度
    if (aHasFlags) {
      int anExtraLength =
          mCurrentWave / aNumWavesPerFlag * 12; // 归还已刷新的旗帜波分割的长度
      aCurrentWaveLength += anExtraLength;
      aNextWaveLength += anExtraLength;
    }

    // 根据倒计时初步计算当前波已经过的比例
    float aFraction = (mZombieCountDownStart - mZombieCountDown) /
                      (float)mZombieCountDownStart;
    if (mZombieHealthToNextWave != -1) {
      // 取得本波僵尸的当前血量
      int aHealthCurrent = TotalZombiesHealthInWave(mCurrentWave - 1);
      // 取得（本波开始时的僵尸总血量 -
      // 下一波刷新时的僵尸总血量），即：本波刷新需要对僵尸造成的伤害
      int aDamageTarget =
          mZombieHealthWaveStart -
          mZombieHealthToNextWave; // 开始时的血量 - 刷新时的血量
      if (aDamageTarget < 1) {
        aDamageTarget = 1; // 需要的伤害至少为 1
      }
      // 再次以刷新血量计算一次当前波已经过的比例
      // 血量比例 = [目标伤害 - (当前血量 - 刷新血量)] / 目标伤害 = (目标伤害 -
      // 仍需造成的伤害) / 目标伤害 = 当前伤害 / 目标伤害
      float aHealthFraction =
          (aDamageTarget - aHealthCurrent + mZombieHealthToNextWave) /
          (float)aDamageTarget;
      // 最终比例取上述二者的较大值
...
void Board::UpdateTutorial() {
  ...
  if (mTutorialState == TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER &&
      mTutorialTimer == 0) {
    DisplayAdvice(_S("[ADVICE_CLICK_PEASHOOTER]"),
                  MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL1_STAY,
                  AdviceType::ADVICE_NONE);
    ...
    mTutorialTimer = -1;
             ...
             mTutorialState ==
                 TutorialState::TUTORIAL_LEVEL_2_REFRESH_SUNFLOWER) {
    if (mTutorialTimer == 0) {
      DisplayAdvice(_S("[ADVICE_PLANT_SUNFLOWER2]"),
                    MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL2,
                    AdviceType::ADVICE_NONE);
      mTutorialTimer = -1;
    } else if (mZombieCountDown == 750 && mCurrentWave == 0) {
      DisplayAdvice(_S("[ADVICE_PLANT_SUNFLOWER3]"),
                    MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL2,
                    AdviceType::ADVICE_NONE);
    }
  } else if (mTutorialState ==
  ...
  if (mApp->IsFirstTimeAdventureMode() && mLevel >= 3 && mLevel != 5 &&
      mLevel <= 7 && mTutorialState == TutorialState::TUTORIAL_OFF &&
      mCurrentWave >= 5 && !gShownMoreSunTutorial &&
      mSeedBank->mSeedPackets[1].CanPickUp() &&
      CountPlantByType(SeedType::SEED_SUNFLOWER) < 3) {
    TOD_ASSERT(!ChooseSeedsOnCurrentLevel());
    DisplayAdvice(_S("[ADVICE_PLANT_SUNFLOWER4]"),
...
void Board::SetTutorialState(TutorialState theTutorialState) {
  ...
  case TutorialState::TUTORIAL_LEVEL_1_PICK_UP_PEASHOOTER:
    if (mPlants.mSize == 0) {
      ...
      DisplayAdvice(_S("[ADVICE_ENOUGH_SUN]"),
                    MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL1_STAY,
                    AdviceType::ADVICE_NONE);
      mTutorialTimer = 400;
    }
  ...
  case TutorialState::TUTORIAL_LEVEL_1_COMPLETED:
    DisplayAdvice(_S("[ADVICE_ZOMBIE_ONSLAUGHT]"),
  ...
  case TutorialState::TUTORIAL_SHOVEL_PICKUP: {
    DisplayAdvice(_S("[ADVICE_CLICK_SHOVEL]"),
  ...
  case TutorialState::TUTORIAL_SHOVEL_DIG:
    DisplayAdvice(_S("[ADVICE_CLICK_PLANT]"),
                  MessageStyle::MESSAGE_STYLE_HINT_STAY,
                  AdviceType::ADVICE_NONE);
    TutorialArrowRemove();
    break;
  ...
  case TutorialState::TUTORIAL_SHOVEL_KEEP_DIGGING:
    DisplayAdvice(_S("[ADVICE_KEEP_DIGGING]"),
                  MessageStyle::MESSAGE_STYLE_HINT_STAY,
                  AdviceType::ADVICE_NONE);
    break;
...
void Board::UpdateGame() {
  ...
  if (StageHasFog() && mFogBlownCountDown > 0) {
    ...
    if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO) {
      mFogOffset =
          TodAnimateCurveFloat(200, 0, mFogBlownCountDown, aMaxFogOffset, 0,
                               TodCurves::CURVE_EASE_OUT);
    } else if (mFogBlownCountDown < 2000) {
      mFogOffset =
          TodAnimateCurveFloat(2000, 0, mFogBlownCountDown, aMaxFogOffset, 0,
                               TodCurves::CURVE_EASE_OUT);
  ...
  if (mApp->mGameScene != GameScenes::SCENE_PLAYING &&
      !mCutScene->ShouldRunUpsellBoard())
    return;
  ...
  if (mMainCounter == 1 && mApp->IsFirstTimeAdventureMode()) {
    if (mLevel == 1) {
      ...
      SetTutorialState(TutorialState::TUTORIAL_LEVEL_2_PICK_UP_SUNFLOWER);
      DisplayAdvice(_S("[ADVICE_PLANT_SUNFLOWER1]"),
                    MessageStyle::MESSAGE_STYLE_TUTORIAL_LEVEL2,
                    AdviceType::ADVICE_NONE);
      mTutorialTimer = 500;
    }
...
void Board::Update() {
  ...
  if (!mSpeedUpButton->mBtnNoDraw) {
    mSpeedUpButton->mDisabled =
        aDisabled || mApp->mGameScene != GameScenes::SCENE_PLAYING;
  }
  mSpeedUpButton->Update();

  if (mSpeedUpFlashCounter > 0) {
    mSpeedUpFlashCounter--;
  }

  if (!mAutoCollectSunButton->mBtnNoDraw) {
    mAutoCollectSunButton->mDisabled =
        aDisabled || mApp->mGameScene != GameScenes::SCENE_PLAYING;
  }
  mAutoCollectSunButton->Update();

  if (mAutoCollectSunFlashCounter > 0) {
    mAutoCollectSunFlashCounter--;
  }
  ...
  if (mShakeCounter > 0) {
    ...
    if (mShakeCounter == 0) {
      ...
      mX = TodAnimateCurve(12, 0, mShakeCounter, 0, mShakeAmountX,
                           TodCurves::CURVE_BOUNCE);
      mY = TodAnimateCurve(12, 0, mShakeCounter, 0, mShakeAmountY,
                           TodCurves::CURVE_BOUNCE);
    }
  }
  ...
  mEffectCounter++;
  if (StageHasPool() && !mIceTrapCounter &&
      mApp->mGameScene != GameScenes::SCENE_ZOMBIES_WON &&
  ...
  if (mBackground == BackgroundType::BACKGROUND_3_POOL &&
      ...
      mDrawCount > 0) {
    int aRenderPosition =
        MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, 2, 0);
    TodParticleSystem *aPoolParticle = mApp->AddTodParticle(
        450, 295, aRenderPosition, ParticleEffect::PARTICLE_POOL_SPARKLY);
...
bool Board::RowCanHaveZombies(int theRow) {
  ...
  return (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RESODDED &&
          theRow <= 4) ||
...
void Board::DrawBackdrop(Graphics *g) {
  ...
  if (mLevel == 1 && mApp->IsFirstTimeAdventureMode()) {
    ...
    int aWidth = TodAnimateCurve(0, 1000, mSodPosition, 0,
                                 Sexy::IMAGE_SOD1ROW->GetWidth(),
                                 TodCurves::CURVE_LINEAR);
    ...
    int aWidth =
        TodAnimateCurve(0, 1000, mSodPosition, 0, 773, TodCurves::CURVE_LINEAR);
...
// 0x416690
bool RenderItemSortFunc(const RenderItem &theItem1,
                        const RenderItem &theItem2) {
...
void Board::AddBossRenderItem(RenderItem *theRenderList, int &theCurRenderItem,
  ...
  if (theBossZombie->IsDeadOrDying()) {
    ...
    Reanimation *aBossReanim =
        mApp->ReanimationTryToGet(theBossZombie->mBodyReanimID);
  ...
  aItem->mZPos =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_BOSS, aBackLegRow, 2);
  aItem->mBossPart = BossPart::BOSS_PART_BACK_LEG;
  theCurRenderItem++;
  aItem = &theRenderList[theCurRenderItem];
  aItem->mRenderObjectType = RenderObjectType::RENDER_ITEM_BOSS_PART;
  aItem->mZPos =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_BOSS, aFrontLegRow, 2);
  ...
  aItem->mRenderObjectType = RenderObjectType::RENDER_ITEM_BOSS_PART;
  aItem->mZPos =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_BOSS, aBackArmRow, 3);
  aItem->mBossPart = BossPart::BOSS_PART_BACK_ARM;
  theCurRenderItem++;

  Reanimation *aBallReanim =
      mApp->ReanimationTryToGet(theBossZombie->mBossFireBallReanimID);
  if (aBallReanim) {
    RenderItem *aItem = &theRenderList[theCurRenderItem];
    aItem->mRenderObjectType = RenderObjectType::RENDER_ITEM_BOSS_PART;
...
static inline void AddGameObjectRenderItemCursorPreview(
    RenderItem *theRenderList, int &theCurRenderItem,
...
static inline void
AddGameObjectRenderItemPlant(RenderItem *theRenderList, int &theCurRenderItem,
                             RenderObjectType theRenderObjectType,
                             GameObject *theGameObject) {
  TOD_ASSERT(theCurRenderItem < MAX_RENDER_ITEMS);
  RenderItem &aRenderItem = theRenderList[theCurRenderItem];
...
static inline void
AddGameObjectRenderItemZombie(RenderItem *theRenderList, int &theCurRenderItem,
                              RenderObjectType theRenderObjectType,
                              GameObject *theGameObject) {
  TOD_ASSERT(theCurRenderItem < MAX_RENDER_ITEMS);
  RenderItem &aRenderItem = theRenderList[theCurRenderItem];
...
static inline void
AddGameObjectRenderItemCoin(RenderItem *theRenderList, int &theCurRenderItem,
                            RenderObjectType theRenderObjectType,
                            GameObject *theGameObject) {
  TOD_ASSERT(theCurRenderItem < MAX_RENDER_ITEMS);
  RenderItem &aRenderItem = theRenderList[theCurRenderItem];
...
void Board::DrawGameObjects(Graphics *g) {
    ...
    while (IteratePlants(aPlant)) {
      if (aPlant->mOnBungeeState == PlantOnBungeeState::NOT_ON_BUNGEE) {
        AddGameObjectRenderItemPlant(aRenderList, aRenderItemCount,
                                     RenderObjectType::RENDER_ITEM_PLANT,
                                     aPlant);
        ...
        if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
          ...
          aRenderItem.mRenderObjectType =
              RenderObjectType::RENDER_ITEM_PLANT_OVERLAY;
          aRenderItem.mZPos =
    ...
    while (IterateZombies(aZombie)) {
      if (aZombie->IsBoss()) {
        ...
        AddGameObjectRenderItemZombie(aRenderList, aRenderItemCount,
                                      RenderObjectType::RENDER_ITEM_ZOMBIE,
                                      aZombie);

        if (aZombie->HasShadow()) {
          RenderItem &aRenderItem = aRenderList[aRenderItemCount];
          aRenderItem.mRenderObjectType =
              RenderObjectType::RENDER_ITEM_ZOMBIE_SHADOW;
          aRenderItem.mZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND,
                                              aZombie->mRow, 3);
        ...
        if (aZombie->mZombieType == ZombieType::ZOMBIE_BUNGEE) {
          ...
          aRenderItem.mZPos = MakeRenderOrder(
              RenderLayer::RENDER_LAYER_PROJECTILE, aZombie->mRow, 1);
          aRenderItem.mZombie = aZombie;
          aRenderItemCount++;
        }
      }
  ...
  }
  {
    ...
    while (IterateProjectiles(aProjectile)) {
      AddGameObjectRenderItemProjectile(
          aRenderList, aRenderItemCount,
          RenderObjectType::RENDER_ITEM_PROJECTILE, aProjectile);

      RenderItem &aRenderItem = aRenderList[aRenderItemCount];
      aRenderItem.mRenderObjectType =
          RenderObjectType::RENDER_ITEM_PROJECTILE_SHADOW;
      aRenderItem.mZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND,
                                          aProjectile->mRow, 3);
  ...
  }
  {
    ...
    while (IterateReanimations(aReanimation)) {
      if (!aReanimation->mIsAttachment) {
        RenderItem &aRenderItem = aRenderList[aRenderItemCount];
        aRenderItem.mRenderObjectType =
            RenderObjectType::RENDER_ITEM_REANIMATION;
  ...
  }
  {
    ...
    while (IterateGridItems(aGridItem)) {
      ...
      if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
        ...
        RenderItem &aRenderItem = aRenderList[aRenderItemCount];
        aRenderItem.mRenderObjectType =
            RenderObjectType::RENDER_ITEM_GRID_ITEM_OVERLAY;
        aRenderItem.mZPos = MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE,
                                            0, aGridItem->mPosY - 30.0f);
        aRenderItem.mGridItem = aGridItem;
        aRenderItemCount++;
      }
    }
  ...
  }
  {
    ...
    } else if (mCutScene->IsAfterSeedChooser() ||
               mCutScene->IsInShovelTutorial() ||
  ...
  if (mApp->mGameScene == GameScenes::SCENE_ZOMBIES_WON) {
    ...
    AddUIRenderItem(aRenderList, aRenderItemCount,
                    RenderObjectType::RENDER_ITEM_DOOR_MASK, aZPos);
  }
  if (StageHasFog()) {
    AddUIRenderItem(aRenderList, aRenderItemCount,
  ...
  if (mApp->IsStormyNightLevel() ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS) {
    AddUIRenderItem(aRenderList, aRenderItemCount,
                    RenderObjectType::RENDER_ITEM_STORM_BACKGROUND,
                    MakeRenderOrder(RenderLayer::RENDER_LAYER_LAWN, 0, 0));
    AddUIRenderItem(aRenderList, aRenderItemCount,
  ...
  AddGameObjectRenderItemCursorPreview(
      aRenderList, aRenderItemCount,
      RenderObjectType::RENDER_ITEM_CURSOR_PREVIEW, mCursorPreview);
  ...
  for (int i = 0; i < aRenderItemCount; i++) {
    ...
    case RenderObjectType::RENDER_ITEM_STORM_BACKGROUND:
      mChallenge->DrawWeatherBackground(g);
      break;

    case RenderObjectType::RENDER_ITEM_STORM:
      mChallenge->DrawWeatherForeground(g);
      break;
...
bool Board::HasProgressMeter() {
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
      mApp->IsFinalBossLevel() || mApp->IsSlotMachineLevel() ||
      mApp->IsSquirrelLevel() || mApp->IsIZombieLevel())
    return true;
...
bool Board::ProgressMeterHasFlags() {
  ...
  if (mApp->IsWhackAZombieLevel() || mApp->IsFinalBossLevel() ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
      mApp->IsSlotMachineLevel() || mApp->IsSquirrelLevel() ||
      mApp->IsIZombieLevel())
    return false;
...
void Board::DrawProgressMeter(Graphics *g) {
  ...
  int aClipWidth =
      TodAnimateCurve(0, PROGRESS_METER_COUNTER, mProgressMeterWidth, 0, 143,
                      TodCurves::CURVE_LINEAR);
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
    ...
    SexyString aMatchStr =
        StrFormat(_S("%d / %d %s"), mChallenge->mChallengeScore, 75,
                  TodStringTranslate(_S("[MATCHES]")).c_str());
    TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12,
                  aColor, DrawStringJustification::DS_ALIGN_CENTER);
  } else if (mApp->IsSquirrelLevel()) {
    SexyString aMatchStr =
        StrFormat(_S("%d / %d %s"), mChallenge->mChallengeScore, 7,
                  TodStringTranslate(_S("[SQUIRRELS]")).c_str());
    TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12,
                  aColor, DrawStringJustification::DS_ALIGN_CENTER);
  } else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_SLOT_MACHINE) {
    int aSunMoney = ClampInt(mSunMoney, 0, 2000);
    SexyString aMatchStr = StrFormat(_S("%d / %d %s"), aSunMoney, 2000,
                                     TodStringTranslate(_S("[SUN]")).c_str());
    TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12,
                  aColor, DrawStringJustification::DS_ALIGN_CENTER);
  } else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM) {
    int aSunMoney = ClampInt(mSunMoney, 0, 1000);
    SexyString aMatchStr = StrFormat(_S("%d / %d %s"), aSunMoney, 1000,
                                     TodStringTranslate(_S("[SUN]")).c_str());
    TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12,
                  aColor, DrawStringJustification::DS_ALIGN_CENTER);
  } else if (mApp->IsIZombieLevel()) {
    SexyString aMatchStr =
        StrFormat(_S("%d / %d %s"), mChallenge->mChallengeScore, 5,
                  TodStringTranslate(_S("[BRAINS]")).c_str());
    TodDrawString(g, aMatchStr, aPosX, 589, Sexy::FONT_DWARVENTODCRAFT12,
                  aColor, DrawStringJustification::DS_ALIGN_CENTER);
    ...
    for (int aFlagWave = 1; aFlagWave <= aNumFlagWaves; aFlagWave++) {
      ...
      if (aTotalWavesAtFlag < mCurrentWave) {
        ...
        aHeight = TodAnimateCurve(100, 0, mFlagRaiseCounter, 0, 14,
                                  TodCurves::CURVE_LINEAR);
      }
      // 计算旗帜的横坐标
      int aPosX = TodAnimateCurve(0, mNumWaves, aTotalWavesAtFlag, aFlagsPosEnd,
                                  606, TodCurves::CURVE_LINEAR);
      // 绘制旗杆
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM ||
      mApp->IsSquirrelLevel() || mApp->IsSlotMachineLevel() ||
      mApp->IsIZombieLevel() || mApp->IsFinalBossLevel())
    return;
  // 绘制僵尸头
  int aHeadProgress =
      TodAnimateCurve(0, 150, mProgressMeterWidth, 0, 135, CURVE_LINEAR);
  g->DrawImageCel(Sexy::IMAGE_FLAGMETERPARTS, aCelWidth - aHeadProgress + 580,
                  572, 0, 0);
}
...
void Board::DrawLevel(Graphics *g) {
  ...
  if (mApp->IsAdventureMode()) {
    aLevelStr = TodStringTranslate(_S("[LEVEL]")) + _S("") +
                mApp->GetStageString(mLevel);
  } else {
    ...
    if (mApp->IsSurvivalMode() ||
      ...
      if (aFlags > 0) {
        SexyString aFlagStr =
            mApp->Pluralize(aFlags, _S("[ONE_FLAG]"), _S("[COUNT_FLAGS]"));
        SexyString aCompletedStr =
            TodReplaceString(_S("[FLAGS_COMPLETED]"), _S("{FLAGS}"), aFlagStr);
        aLevelStr =
            StrFormat(_S("%s - %s"), TodStringTranslate(aLevelStr).c_str(),
                      aCompletedStr.c_str());
      }
    } else if (mApp->IsEndlessIZombie(mApp->mGameMode) ||
               mApp->IsEndlessScaryPotter(mApp->mGameMode)) {
      ...
      if (aStreak > 0) {
        SexyString aStreakStr = TodReplaceNumberString(_S("[ENDLESS_STREAK]"),
                                                       _S("{STREAK}"), aStreak);
        aLevelStr =
            StrFormat(_S("%s - %s"), TodStringTranslate(aLevelStr).c_str(),
                      aStreakStr.c_str());
  ...
  if (mChallenge->mChallengeState ==
      ChallengeState::STATECHALLENGE_ZEN_FADING) {
    aPosY += TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 0, 50,
                             TodCurves::CURVE_EASE_IN_OUT);
  }
  TodDrawString(g, aLevelStr, aPosX, aPosY, Sexy::FONT_HOUSEOFTERROR16,
                Color(224, 187, 98), DrawStringJustification::DS_ALIGN_RIGHT);
}
...
void Board::DrawZenWheelBarrowButton(Graphics *g, int theOffsetY) {
  ...
  if (aPlant && mCursorObject->mCursorType !=
                    CursorType::CURSOR_TYPE_PLANT_FROM_WHEEL_BARROW) {
    if (mChallenge->mChallengeState ==
        ChallengeState::STATECHALLENGE_ZEN_FADING) {
...
void Board::DrawZenButtons(Graphics *g) {
  ...
  if (mChallenge->mChallengeState ==
      ChallengeState::STATECHALLENGE_ZEN_FADING) {
    aOffsetY = TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 0,
                               -72, TodCurves::CURVE_EASE_IN_OUT);
  }
  ...
  for (GameObjectType aTool = GameObjectType::OBJECT_TYPE_WATERING_CAN;
    ...
    if (aTool == GameObjectType::OBJECT_TYPE_NEXT_GARDEN) {
      ...
      GetZenButtonRect(aTool, aButtonRect);
      g->DrawImage(Sexy::IMAGE_SHOVELBANK, aButtonRect.mX,
                   aButtonRect.mY + aOffsetY);
      ...
      if (aTool == GameObjectType::OBJECT_TYPE_WATERING_CAN) {
        ...
        int aCharges = mApp->mPlayerInfo
                           ->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] -
                       PURCHASE_COUNT_OFFSET;
        if (aCharges == 0) {
          g->SetColorizeImages(true);
          g->SetColor(Color(96, 96, 96));
        } else if (mTutorialState ==
                   TutorialState::TUTORIAL_ZEN_GARDEN_FERTILIZE_PLANTS) {
          g->SetColorizeImages(true);
        ...
        TodDrawString(g, aChargeString, aButtonRect.mX + 64,
        ...
        int aCharges = mApp->mPlayerInfo
                           ->mPurchases[(int)StoreItem::STORE_ITEM_BUG_SPRAY] -
                       PURCHASE_COUNT_OFFSET;
        if (aCharges == 0) {
          g->SetColorizeImages(true);
          g->SetColor(Color(128, 128, 128));
        }
        g->DrawImage(Sexy::IMAGE_BUG_SPRAY, aButtonRect.mX,
        ...
        int aCharges = mApp->mPlayerInfo
                           ->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] -
                       PURCHASE_COUNT_OFFSET;
        if (aCharges == 0) {
          g->SetColorizeImages(true);
          g->SetColor(Color(128, 128, 128));
        ...
        int aCharges = mApp->mPlayerInfo
                           ->mPurchases[(int)StoreItem::STORE_ITEM_TREE_FOOD] -
                       PURCHASE_COUNT_OFFSET;
        if (aCharges <= 0) {
          g->SetColorizeImages(true);
          g->SetColor(Color(128, 128, 128));
  ...
  if (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN &&
    ...
    if (mShowShovel) {
      ...
      if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_SHOVEL) {
        ...
        g->DrawImage(Sexy::IMAGE_SHOVEL, aShovelRect.mX - 7,
                     aShovelRect.mY - 3);
        g->SetColorizeImages(false);
      }
    }
...
void Board::DrawDebugText(Graphics *g) {
  ...
  case DebugTextMode::DEBUG_TEXT_ZOMBIE_SPAWN: {
    ...
    aText += StrFormat(_S("TimeSinseLastSpawn: %d %s\n"), aTime,
                       aTime > 400 ? _S("") : _S("(too soon)"));
    aText +=
        StrFormat(_S("ZombieCountDown: %d/%d (%.0f%%)\n"), mZombieCountDown,
                  mZombieCountDownStart, aCountDownFraction);

    if (mZombieHealthToNextWave != -1) {
      int aTotalHealth = TotalZombiesHealthInWave(mCurrentWave - 1);
      int aHealthRange =
          max(mZombieHealthWaveStart - mZombieHealthToNextWave, 1);
      float aHealthFraction =
          (float)(mZombieHealthToNextWave - aTotalHealth + aHealthRange) /
          (float)aHealthRange;
      aText += StrFormat(
  ...
  case DebugTextMode::DEBUG_TEXT_MUSIC: {
    ...
    if (mApp->mMusic->mCurMusicFileMain == MusicFile::MUSIC_FILE_NONE) {
      ...
      } else if (mApp->mMusic->mMusicBurstState ==
                 MusicBurstState::MUSIC_BURST_STARTING) {
        aText += StrFormat(_S("Starting %d/%d"),
                           mApp->mMusic->mBurstStateCounter, 400);
      } else if (mApp->mMusic->mMusicBurstState ==
                 MusicBurstState::MUSIC_BURST_ON) {
        aText += StrFormat(_S("On at least until %d/%d"),
                           mApp->mMusic->mBurstStateCounter, 800);
      } else if (mApp->mMusic->mMusicBurstState ==
                 MusicBurstState::MUSIC_BURST_FINISHING) {
        aText += StrFormat(_S("Finishing %d/%d"),
                           mApp->mMusic->mBurstStateCounter, 400);
      }

      if (mApp->mMusic->mMusicDrumsState == MusicDrumsState::MUSIC_DRUMS_OFF) {
        aText += StrFormat(_S(", Drums off"));
      } else if (mApp->mMusic->mMusicDrumsState ==
                 MusicDrumsState::MUSIC_DRUMS_ON_QUEUED) {
        aText += StrFormat(_S(", Drums queued on"));
      } else if (mApp->mMusic->mMusicDrumsState ==
                 MusicDrumsState::MUSIC_DRUMS_ON) {
        aText += StrFormat(_S(", Drums on"));
      } else if (mApp->mMusic->mMusicDrumsState ==
                 MusicDrumsState::MUSIC_DRUMS_OFF_QUEUED) {
        aText += StrFormat(_S(", Drums queued off"));
      } else if (mApp->mMusic->mMusicDrumsState ==
                 MusicDrumsState::MUSIC_DRUMS_FADING) {
        aText += StrFormat(_S(", Drums fading off %d/%d"),
                           mApp->mMusic->mDrumsStateCounter, 50);
      }
      aText += StrFormat(_S("\n"));

      int aPackedOrderMain =
          mApp->mMusic->GetMusicOrder(mApp->mMusic->mCurMusicFileMain);
      ...
      if (mApp->mMusic->mCurMusicTune == MusicTune::MUSIC_TUNE_DAY_GRASSWALK ||
        ...
        if (aCurrentOrder == LOWORD(aPackedOrderHihats) &&
          ...
          int aDiffHihats =
              HIWORD(aPackedOrderHihats) - HIWORD(aPackedOrderMain);
        ...
        float f1, f2, f3;
        gBass->BASS_ChannelGetAttribute(aMusicHandle1, BASS_ATTRIB_MUSIC_BPM,
                                        &f1);
        gBass->BASS_ChannelGetAttribute(aMusicHandle2, BASS_ATTRIB_MUSIC_BPM,
                                        &f2);
        gBass->BASS_ChannelGetAttribute(aMusicHandle3, BASS_ATTRIB_MUSIC_BPM,
                                        &f3);
        int bpm1 = (int)f1;
        int bpm2 = (int)f2;
        int bpm3 = (int)f3;
        ...
        int aPackedOrderDrums =
            mApp->mMusic->GetMusicOrder(mApp->mMusic->mCurMusicFileDrums);
        aText +=
            StrFormat(_S("Drum order %02d row %02d\n"),
...
void Board::DrawDebugObjectRects(Graphics *g) {
...

  {
    ...
    while (IteratePlants(aPlant)) {
      ...
      Rect aAttackRect =
          aPlant->GetPlantAttackRect(PlantWeapon::WEAPON_PRIMARY);
      ...
      Rect aSecondaryRect =
          aPlant->GetPlantAttackRect(PlantWeapon::WEAPON_SECONDARY);
...
void Board::DrawFadeOut(Graphics *g) {
  if (mSpeedUpFlashCounter > 0) {
    g->SetColor(Color(255, 255, 255, mSpeedUpFlashCounter * 255 / 100));
  ...
  int anAlpha = TodAnimateCurve(200, 0, mBoardFadeOutCounter, 0, 255,
                                TodCurves::CURVE_LINEAR);
  if (mLevel == 9 || mLevel == 19 || mLevel == 29 || mLevel == 39 ||
      mLevel == 49) {
...
void Board::DrawTopRightUI(Graphics *g) {
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
    if (mChallenge->mChallengeState == STATECHALLENGE_ZEN_FADING) {
      mMenuButton->mY =
          TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, -10, -50,
                          TodCurves::CURVE_EASE_IN_OUT);
      mStoreButton->mX =
          TodAnimateCurve(50, 0, mChallenge->mChallengeStateCounter, 678, 800,
                          TodCurves::CURVE_EASE_IN_OUT);
    } else {
      mMenuButton->mY = -10;
      mStoreButton->mX = 678;
...
void Board::DrawUIBottom(Graphics *g) {
  if (mBackground == BackgroundType::BACKGROUND_ZOMBIQUARIUM) {
    ...
    TodDrawImageCelScaled(g, Sexy::IMAGE_WAVESIDE, 800, 40, 0, aWaveTime, -1.0f,
                          1.0f);
  ...
  if (mBackground == BackgroundType::BACKGROUND_GREENHOUSE ||
      mBackground == BackgroundType::BACKGROUND_ZOMBIQUARIUM) {
    g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
    g->DrawImage(IMAGE_BACKGROUND_GREENHOUSE_OVERLAY,
...
void Board::ClearFogAroundPlant(Plant *thePlant, int theSize) {
  // int aFogFadeOutSpeed = mFogBlownCountDown >= 2000 ? 40 : mFogBlownCountDown
  // > 0 ? 2 : 6;
...
void Board::UpdateFog() {
  ...
  // int aFogFadeInSpeed = mFogBlownCountDown >= 2000 ? 20 : mFogBlownCountDown
  // > 0 ? 1 : 3;
...
void Board::DrawFog(Graphics *g) {
  Image *aImageFog =
      mApp->Is3DAccelerated() ? Sexy::IMAGE_FOG : Sexy::IMAGE_FOG_SOFTWARE;
  for (int x = 0; x < MAX_GRID_SIZE_X; x++) {
    for (int y = 0; y < MAX_GRID_SIZE_Y + 1; y++) {
      ...
      // { sub eax,edx } 向前 [y / 6] 列，但 y 超出上限 y - 5
      // 行，故相当于列不变，行 = y % 6；
      ...
      // 根据初相和时间计算当前相位
      float aMotion =
          13 + 4 * sin(aTime / 900 + aPhaseY) + 8 * sin(aTime / 500 + aPhaseX);
      ...
      g->SetColorizeImages(true);
      g->SetColor(
          Color(aColorVariant, aColorVariant, aLightnessVariant, aFadeAmount));
...
void Board::DoTypingCheck(KeyCode theKey) {
  ...
  if (mApp->mMustacheCheck->Check(theKey) ||
      mApp->mMoustacheCheck->Check(theKey)) {
    SetMustacheMode(!mMustacheMode);
    return;
  }
  if (mApp->mSuperMowerCheck->Check(theKey) ||
      mApp->mSuperMowerCheck2->Check(theKey)) {
  ...
  if (mApp->mPinataCheck->Check(theKey)) {
    if (mApp->CanDoPinataMode()) {
      ...
      if (mApp->mGameScene == GameScenes::SCENE_PLAYING) {
        DisplayAdvice(_S("[CANT_USE_CODE]"),
...
bool Board::HandleHotkey(KeyCode theKey)
{
  if (mApp->GetDialogCount() > 0)
    return false;

  uint8_t mods = HotkeyGetMods();

  for (int i = 0; i < gHotkeyCount; i++)
  {
    const Hotkey& h = gHotkeys[i];
    if (!HotkeyMatches(h, theKey, mods))
      continue;

    bool isPlaying = (mApp->mGameScene == GameScenes::SCENE_PLAYING);

    switch (h.mAction)
    {
    case HotkeyAction::Pause:
      if (isPlaying && mApp->CanPauseNow())
      {
        mApp->PlaySample(Sexy::SOUND_PAUSE);
        mApp->DoPauseDialog();
        return true;
    ...
    case HotkeyAction::ToggleAutoCollectSun:
      if (isPlaying && !mPaused)
      {
        mAutoCollectSunEnabled = !mAutoCollectSunEnabled;
        mAutoCollectSunButton->SetLabel(mAutoCollectSunEnabled ? _S("AUTO*") : _S("AUTO"));
    ...
    case HotkeyAction::ToggleShovel:
      if (isPlaying && !mPaused)
      {
        if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_SHOVEL)
        {
        ...
        else if (CanUseGameObject(GameObjectType::OBJECT_TYPE_SHOVEL))
        {
    ...
    case HotkeyAction::ShowHelp:
    {
      HotkeyHelpDialog* aDialog = new HotkeyHelpDialog(mApp);
      mApp->AddDialog(Dialogs::DIALOG_HOTKEY_HELP, aDialog);
    ...
    default:
      ...
      {
        if (isPlaying && !mPaused && mSeedBank)
        {
          int idx = (int)h.mAction - (int)HotkeyAction::SelectSeed1;
          HotkeySelectSeedPacket(idx);
          return true;
        }
        return false;
      }
...
void Board::HotkeySelectSeedPacket(int theIndex)
{
  if (theIndex < 0 || theIndex >= mSeedBank->mNumPackets)
  {
    mApp->PlaySample(Sexy::SOUND_BUZZER);
    return;
  }

  SeedPacket& aPacket = mSeedBank->mSeedPackets[theIndex];

  if (mApp->IsSlotMachineLevel() || mApp->IsIZombieLevel())
  {
    mApp->PlaySample(Sexy::SOUND_BUZZER);
    return;
  }
  ...
  mCursorObject->mType = aPacket.mPacketType;
  mCursorObject->mImitaterType = aPacket.mImitaterType;
  mCursorObject->mCursorType = CursorType::CURSOR_TYPE_PLANT_FROM_BANK;
  mCursorObject->mSeedBankIndex = theIndex;
  mApp->PlaySample(SOUND_SEEDLIFT);
  aPacket.Deactivate();
}

void Board::RequestRestartLevel()
{
  SexyString aTitle;
  SexyString aMessage;
  if (mApp->IsPuzzleMode())
  {
    aTitle = _S("[RESTART_PUZZLE_HEADER]");
    aMessage = _S("[RESTART_PUZZLE_BODY]");
  }
  else if (mApp->IsChallengeMode())
  {
    aTitle = _S("[RESTART_CHALLENGE_HEADER]");
    aMessage = _S("[RESTART_CHALLENGE_BODY]");
  }
  else if (mApp->IsSurvivalMode())
  {
    aTitle = _S("[RESTART_SURVIVAL_HEADER]");
    aMessage = _S("[RESTART_SURVIVAL_BODY]");
  }
  else
  {
    aTitle = _S("[RESTART_LEVEL_HEADER]");
    aMessage = _S("[RESTART_LEVEL_BODY]");
  }

  LawnDialog* aDialog = (LawnDialog*)mApp->DoDialog(
      Dialogs::DIALOG_CONFIRM_RESTART, true, aTitle, aMessage, _S(""),
      Dialog::BUTTONS_YES_NO);
  aDialog->mLawnYesButton->mLabel = TodStringTranslate(_S("RESTART"));
...
void Board::KeyDown(KeyCode theKey) {
  ...
  } else if (HandleHotkey(theKey)) {
    return;
  } else if (theKey == KeyCode::KEYCODE_RETURN ||
             theKey == KeyCode::KEYCODE_SPACE) {
...
void Board::KeyChar(SexyChar theChar) {
  ...
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN) {
    if (theChar == _S('m')) {
      if (!mApp->mZenGarden->IsZenGardenFull(true)) {
        ...
        aPottedPlant.mDrawVariation = (DrawVariation)RandRangeInt(
            (int)DrawVariation::VARIATION_MARIGOLD_WHITE,
    ...
    if (theChar == _S('+')) {
      if (!mApp->mZenGarden->IsZenGardenFull(true)) {
        PottedPlant aPottedPlant;
        aPottedPlant.InitializePottedPlant(
            mApp->mZenGarden->PickRandomSeedType());
        mApp->mZenGarden->AddPottedPlant(&aPottedPlant);
      }
      return;
    }
    ...
    if (theChar == _S('r')) {
      ...
      while (IteratePlants(aPlant)) {
        if (aPlant->mPottedPlantIndex >= 0) {
          TOD_ASSERT(aPlant->mPottedPlantIndex <
    ...
    if (mApp->IsSurvivalEndless(mApp->mGameMode)) {
      ...
      for (int y = 0; y < MAX_GRID_SIZE_X; y++) {
        for (int x = 0; x < MAX_GRID_SIZE_Y; x++) {
          if (CanPlantAt(x, y, SeedType::SEED_LILYPAD) ==
              PlantingReason::PLANTING_OK) {
          ...
          if (CanPlantAt(x, y, SeedType::SEED_GATLINGPEA) ==
            ...
            if (x < 5) {
              ...
              if (CanPlantAt(x, y, SeedType::SEED_INSTANT_COFFEE) ==
                  PlantingReason::PLANTING_OK) {
                AddPlant(x, y, SeedType::SEED_INSTANT_COFFEE,
                         SeedType::SEED_NONE);
              }
            }
          }
      ...
      for (int y = 0; y < MAX_GRID_SIZE_Y; ++y) {
        for (int x = 0; x < MAX_GRID_SIZE_X; ++x) {
          if (StageHasRoof() && CanPlantAt(x, y, SeedType::SEED_FLOWERPOT) ==
                                    PlantingReason::PLANTING_OK) {
  ...
  if (theChar == _S('O')) {
    ...
    for (int y = 0; y < MAX_GRID_SIZE_Y; y++) {
      for (int x = 0; x < 3; x++) {
        if (CanPlantAt(x, y, SeedType::SEED_FLOWERPOT) ==
            PlantingReason::PLANTING_OK) {
          AddPlant(x, y, SeedType::SEED_FLOWERPOT, SeedType::SEED_NONE);
        }
      }
  ...
  if (theChar == _S('D')) {
    AddZombie(ZombieType::ZOMBIE_DASH_AOE, Zombie::ZOMBIE_WAVE_DEBUG);
    return;
  }
  if (theChar == _S('G')) {
...
bool Board::HasConveyorBeltSeedBank() {
  return mApp->IsFinalBossLevel() || mApp->IsMiniBossLevel() ||
         mApp->IsShovelLevel() || mApp->IsWallnutBowlingLevel() ||
         mApp->IsLittleTroubleLevel() || mApp->IsStormyNightLevel() ||
         mApp->IsBungeeBlitzLevel() ||
         mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT ||
         mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN ||
         mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_INVISIGHOUL;
...
bool Board::StageHasZombieWalkInFromRight() {
  if (mApp->IsWhackAZombieLevel() ||
      ...
      mApp->mGameMode == GameMode::GAMEMODE_TREE_OF_WISDOM ||
      mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBIQUARIUM ||
      mApp->IsFinalBossLevel() || mApp->IsIZombieLevel() ||
...
int Board::PlantingPixelToGridY(int theX, int theY, SeedType theSeedType) {
  ...
  if (theSeedType == SeedType::SEED_INSTANT_COFFEE) {
    ...
    if (aGridYDown != aGridY) {
      Plant *aPlantDown = GetTopPlantAt(
          aGridX, aGridYDown, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
    ...
    if (aGridYUp != aGridY) {
      Plant *aPlantUp = GetTopPlantAt(
          aGridX, aGridYUp, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
...
int Board::GridToPixelY(int theGridX, int theGridY) {
  ...
  if (theGridX != -1 && mGridSquareType[theGridX][theGridY] ==
                            GridSquareType::GRIDSQUARE_HIGH_GROUND) {
...
// 0x41C850
bool GetCircleRectOverlap(int theCircleX, int theCircleY, int theRadius,
                          const Rect &theRect) {
...
bool Board::IterateParticles(TodParticleSystem *&theParticle) {
  while (mApp->mEffectSystem->mParticleHolder->mParticleSystems.IterateNext(
      theParticle)) {
...
bool Board::IterateReanimations(Reanimation *&theReanimation) {
  while (mApp->mEffectSystem->mReanimationHolder->mReanimations.IterateNext(
      theReanimation)) {
...
unsigned int Board::SeedNotRecommendedForLevel(SeedType theSeedType) {
  ...
  if (StageHasRoof() && (theSeedType == SeedType::SEED_SPIKEWEED ||
                         theSeedType == SeedType::SEED_SPIKEROCK)) {
...
void Board::DropLootPiece(int thePosX, int thePosY, int theDropFactor) {
  if (mApp->IsFirstTimeAdventureMode()) {
    if (mLevel == 22 && mCurrentWave > 5 &&
        !mApp->mPlayerInfo->mHasUnlockedMinigames &&
    ...
    if (mLevel == 36 && mCurrentWave > 5 &&
        !mApp->mPlayerInfo->mHasUnlockedPuzzleMode &&
  ...
  if (mApp->IsWhackAZombieLevel()) {
    ...
    if (aDropHit >= aSunChanceMin * theDropFactor &&
      ...
      AddCoin(thePosX - 20, thePosY, CoinType::COIN_SUN,
              CoinMotion::COIN_MOTION_COIN);
      AddCoin(thePosX - 40, thePosY, CoinType::COIN_SUN,
              CoinMotion::COIN_MOTION_COIN);
      AddCoin(thePosX - 60, thePosY, CoinType::COIN_SUN,
              CoinMotion::COIN_MOTION_COIN);
      return;
    }
  }
  ...
  if (mApp->mZenGarden->CanDropChocolate()) {
    if (mApp->IsAdventureMode() && !mApp->IsFirstTimeAdventureMode()) {
      ...
      aChocolateChance = aPottedPlantChance +
                         (mApp->IsSurvivalEndless(mApp->mGameMode) ? 9 : 36);
    }
  }
...
Plant *Board::FindUmbrellaPlant(int theGridX, int theGridY) {
  ...
  while (IteratePlants(aPlant)) {
    if (aPlant->mSeedType == SeedType::SEED_UMBRELLA &&
        !aPlant->NotOnGround() &&
        GridInRange(theGridX, theGridY, aPlant->mPlantCol, aPlant->mRow, 1,
                    1)) {
      return aPlant;
    }
  }
...
void Board::DoFwoosh(int theRow) {
  int aRenderOrder =
      MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, theRow, 1);
  for (int i = 0; i < 12; i++) {
    ...
    Reanimation *aFwoosh = mApp->AddReanimation(
        aPosX, aPosY, aRenderOrder, ReanimationType::REANIM_JALAPENO_FIRE);
...
void Board::UpdateFwoosh() {
  ...
  int aFwooshRemaining = TodAnimateCurve(50, 0, --mFwooshCountDown, 12, 0,
                                         TodCurves::CURVE_LINEAR);
  for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++) {
...
void Board::UpdateGridItems() {
  ...
  while (IterateGridItems(aGridItem)) {
    if (mEnableGraveStones &&
        aGridItem->mGridItemType == GridItemType::GRIDITEM_GRAVESTONE &&
        aGridItem->mGridItemCounter < 100) {
...
// 0x41D8A0
void Board::KillAllZombiesInRadius(int theRow, int theX, int theY,
                                   int theRadius, int theRowRange, bool theBurn,
  ...
  while (IterateGridItems(aGridItem)) {
    if (aGridItem->mGridItemType == GridItemType::GRIDITEM_LADDER) {
      if (GridInRange(aGridItem->mGridX, aGridItem->mGridY, aGridX, aGridY,
                      theRowRange, theRowRange)) {
        aGridItem->GridItemDie();
      }
    }
  }
...
int Board::GetNumWavesPerSurvivalStage() {
  if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND ||
    ...
    return 10;
             ...
             mApp->IsSurvivalEndless(mApp->mGameMode)) {
    return 20;
  }
...
bool Board::PlantUsesAcceleratedPricing(SeedType theSeedType) {
  return Plant::IsUpgrade(theSeedType) &&
         mApp->IsSurvivalEndless(mApp->mGameMode);
}
...
bool Board::CanUseGameObject(GameObjectType theGameObject) {
  ...
  if (theGameObject == GameObjectType::OBJECT_TYPE_NEXT_GARDEN) {
    return mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_MUSHROOM_GARDEN] ||
           mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_AQUARIUM_GARDEN] ||
           mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_TREE_OF_WISDOM];
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_FERTILIZER) {
    return mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] > 0;
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_BUG_SPRAY) {
    return mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_BUG_SPRAY] >
           0;
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_PHONOGRAPH) {
    return mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_PHONOGRAPH] > 0;
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_CHOCOLATE) {
    return mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_CHOCOLATE] >
           0;
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_WHEELBARROW) {
    return mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_WHEEL_BARROW] > 0;
  }
  if (theGameObject == GameObjectType::OBJECT_TYPE_GLOVE) {
    return mApp->mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_GARDENING_GLOVE] > 0;
  }
...
int Board::NumberZombiesInWave(int theWaveIndex) {
  TOD_ASSERT(theWaveIndex >= 0 && theWaveIndex < MAX_ZOMBIE_WAVES &&
             theWaveIndex < mNumWaves);
