...
namespace
{
	// IDs for the top-right speed / auto-collect buttons (recreated from scratch;
	// the original creation block was never ported). Picked clear of other widget ids.
	constexpr int BOARD_SPEEDUP_BUTTON_ID = 3301;
	constexpr int BOARD_AUTOCOLLECT_BUTTON_ID = 3302;

	// Screen-space (camera-relative) anchor for the top-right buttons: their on-screen
	// corner stays fixed regardless of the lawn shake offset (Board::mX/mY). The actual
	// button rect is set to (BASE - shake) each frame so that, after the shake transform,
	// they draw at BASE and hover/click hit-tests (which run in un-shaken widget space)
	// line up with the drawn position.
	constexpr int BOARD_SPEEDUP_BUTTON_W = 100;
	constexpr int BOARD_SPEEDUP_BUTTON_H = 40;
	constexpr int BOARD_SPEEDUP_BUTTON_X = 696;
	constexpr int BOARD_SPEEDUP_BUTTON_Y = 38;
	constexpr int BOARD_AUTOCOLLECT_BUTTON_X = 696;
	constexpr int BOARD_AUTOCOLLECT_BUTTON_Y = 82;
}
...
Board::Board(LawnApp* theApp)
{
	...
	// The speed-up and auto-collect buttons' original creation block was never ported,
	// so recreate them here. Like mMenuButton/mStoreButton they are NOT widgets: they are
	// drawn manually in DrawTopRightUI, ticked in Update(), and click-tested manually in
	// MouseUp() (GameButton has no widget/listener input path of its own).
	mSpeedUpButton = new GameButton(BOARD_SPEEDUP_BUTTON_ID);
	mSpeedUpButton->mDrawStoneButton = true;
	mSpeedUpButton->mParentWidget = this;
	mSpeedUpButton->mWidth = BOARD_SPEEDUP_BUTTON_W;
	mSpeedUpButton->mHeight = BOARD_SPEEDUP_BUTTON_H;
	// Position is (re)computed every frame in Update() so the button tracks the camera
	// (stays screen-fixed instead of shaking with the lawn).
	RefreshSpeedUpLabel();

	mAutoCollectSunButton = new GameButton(BOARD_AUTOCOLLECT_BUTTON_ID);
	mAutoCollectSunButton->mDrawStoneButton = true;
	mAutoCollectSunButton->mParentWidget = this;
	mAutoCollectSunButton->mWidth = BOARD_SPEEDUP_BUTTON_W;
	mAutoCollectSunButton->mHeight = BOARD_SPEEDUP_BUTTON_H;
	...
	if (mNextSurvivalStageCounter > 0)
	{
		...
		if (!mNextSurvivalStageCounter)
		{
			...
			else
			{
				...
				// A survival wave/stage just ended: the same Board is reused for the next
				// stage (repick), so clear the toggles here or they'd stay active next wave.
...
bool Board::CanInteractWithBoardButtons()
{
	...
	// Disable the top-right buttons (menu / speed / auto-collect) while a cutscene or
	// the zombies-won sequence is playing. During normal play the scene is SCENE_PLAYING;
	// the shovel tutorial also runs outside SCENE_PLAYING but must stay interactive.
...
void Board::MouseUp(int x, int y, int theClickCount)
{
	...
	{
		// Top-right speed / auto-collect buttons (recreated from scratch; click-tested
		// manually like mMenuButton since GameButton is not a Widget).
		if (mSpeedUpButton && mSpeedUpButton->IsMouseOver())
		{
		...
		if (mAutoCollectSunButton && mAutoCollectSunButton->IsMouseOver())
		{
			mAutoCollectSunButton->mIsOver = false;
			mAutoCollectSunButton->mIsDown = false;
			ToggleAutoCollect();
			return;
		}
...
void Board::PickSpecialGraveStone()
{
	...
	if (aPickCount > 0)
	{
		// NOTE: index the pointer array directly. The original code cast aPicks to
		// (int*) and routed it through TodPickFromArray, which treats elements as
		// 4-byte ints. On x64 a GridItem* is 8 bytes, so that truncated the pointer
		// to 32 bits and crashed on deref. Pick the element with a pointer-sized index.
		aPicks[Sexy::Rand(aPickCount)]->mGridItemState = GridItemState::GRIDITEM_STATE_GRAVESTONE_SPECIAL;
	}
}
...
void Board::Update()
{
	...
	if (mSpeedUpButton)
	{
		...
		// Anchor to the screen corner: cancel the lawn shake (mX/mY) so the button is
		// camera-relative. Set before Update() so hover highlighting uses the same rect.
		mSpeedUpButton->mX = BOARD_SPEEDUP_BUTTON_X - mX;
		mSpeedUpButton->mY = BOARD_SPEEDUP_BUTTON_Y - mY;
		mSpeedUpButton->Update();
		RefreshSpeedUpLabel();
	}
	if (mAutoCollectSunButton)
	{
		mAutoCollectSunButton->mDisabled = aDisabled;
		mAutoCollectSunButton->mX = BOARD_AUTOCOLLECT_BUTTON_X - mX;
		mAutoCollectSunButton->mY = BOARD_AUTOCOLLECT_BUTTON_Y - mY;
...
void Board::DrawTopRightUI(Graphics* g)
{
	...
	mMenuButton->Draw(g);
	// mSpeedUpButton / mAutoCollectSunButton are not widgets (GameButton isn't a Widget),
	// so draw them manually here like mMenuButton. The auto-collect flash is a transient
	// overlay drawn on top.
	if (mSpeedUpButton)
		mSpeedUpButton->Draw(g);
	if (mAutoCollectSunButton)
		mAutoCollectSunButton->Draw(g);
	DrawAutoCollectFlash(g);
	g->SetColorizeImages(false);
...
// Bright white flash over the auto-collect button that fades out quickly when toggled.
// (The speed-up flash is the full-screen flash drawn in DrawFadeOut.)
void Board::DrawAutoCollectFlash(Graphics* g)
{
	if (mAutoCollectSunFlashCounter > 0 && mAutoCollectSunButton)
	{
		int aAlpha = mAutoCollectSunFlashCounter * 255 / 100;
		if (aAlpha > 255)
			aAlpha = 255;
		g->SetColor(Color(255, 255, 255, aAlpha));
		g->FillRect(mAutoCollectSunButton->mX, mAutoCollectSunButton->mY,
			mAutoCollectSunButton->mWidth, mAutoCollectSunButton->mHeight);
	}
}

// Cycle the game speed 1x -> 2x -> 4x -> 1x and trigger the fading flash.
void Board::ToggleSpeedUp()
{
	if (mApp->mUpdateMultiplier < 1.5)
		mApp->mUpdateMultiplier = 2.0;
	else if (mApp->mUpdateMultiplier < 3.0)
		mApp->mUpdateMultiplier = 4.0;
	else
		mApp->mUpdateMultiplier = 1.0;
...
void Board::RefreshSpeedUpLabel()
{
	if (!mSpeedUpButton)
		return;
	int aSpeed = (int)(mApp->mUpdateMultiplier + 0.5);
	if (aSpeed < 1)
		aSpeed = 1;
	mSpeedUpButton->SetLabel(StrFormat(_S("SPEED x%d"), aSpeed));
}
...
void Board::KeyDown(KeyCode theKey)
{
	...
	{
		// QoL hotkey consumed the key (see Lawn/Hotkey.cpp for the full table:
		// pause, fast-forward, auto-sun, seed select, shovel, mute, restart, help, FPS)
	}
...
void Board::RequestRestartLevel()
{
	...
	if (aDialog->WaitForResult(true) == Dialog::ID_YES)
	{
		...
		// Restarting the level: clear the speed / auto-collect toggles so the replayed
		// level starts at 1x with auto-collect off (same as the survival wave-end reset).
		mAutoCollectSunEnabled = false;
		if (mAutoCollectSunButton)
			mAutoCollectSunButton->SetLabel(_S("AUTO"));
		mApp->mUpdateMultiplier = 1.0;
