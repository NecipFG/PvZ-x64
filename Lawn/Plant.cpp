...
PlantDefinition gPlantDefs[SeedType::NUM_SEED_TYPES] = {  //0x69F2B0
    ...
    { SeedType::SEED_CONCEPTPLANT,      nullptr, ReanimationType::REANIM_ZENGARDEN_SPROUT,    0,  150,    4000,   PlantSubClass::SUBCLASS_NORMAL,     0,      _S("CONCEPT_PLANT") },
    { SeedType::SEED_WALLKNIGHT,      nullptr, ReanimationType::REANIM_WALLNUT,    0,  150,    4000,   PlantSubClass::SUBCLASS_NORMAL,     0,      _S("WALLKNIGHT") }
...
void Plant::PlantInitialize(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
    ...
    case SeedType::SEED_TANGLEKELP:
        ...
        aBodyReanim->SetTruncateDisappearingFrames();
        break;
    case SeedType::SEED_CONCEPTPLANT:
...
void Plant::UpdateShooter()
{
    ...
    // ConceptPlant 175% boost: extra decrement to properly decrease the reload delay
    if (mBoostCounter > 0 && mLaunchCounter > 0 && (mBoostCounter % 4) != 0)
        mLaunchCounter--;
...
void Plant::UpdateAbilities()
{
    ...
    else if (mSeedType == SeedType::SEED_CONCEPTPLANT)                                          UpdateConceptPlant();
...
void Plant::UpdateReanimColor()
{
    ...
    else if (mSeedType == SeedType::SEED_CONCEPTPLANT && mShootingCounter > 0)
    {
    ...
    if (mBoostCounter > 0)
    {
        // Smooth sine wave flicker between 128 and 255 brightness
        float aSine = sin(mBoard->mMainCounter * 0.1f); 
        int aFlickerVal = 192 + (int)(aSine * 63); 
        aColorOverride.mRed = (aColorOverride.mRed * aFlickerVal) / 255;
        aColorOverride.mGreen = (aColorOverride.mGreen * aFlickerVal) / 255;
        aColorOverride.mBlue = (aColorOverride.mBlue * aFlickerVal) / 255;
    }
...
{
    if (NotOnGround() || mShootingCounter == 0 || mSeedType == SeedType::SEED_CONCEPTPLANT)
        return;

    mShootingCounter--;

    // ConceptPlant 175% boost: extra decrement 3 of every 4 ticks (average 1.75×)
    // We only trigger this if mShootingCounter > 1, so we don't accidentally skip the mShootingCounter == 1 shot execution frame!
    if (mBoostCounter > 0 && mShootingCounter > 1 && (mBoostCounter % 4) != 0)
        mShootingCounter--;
...
void Plant::Draw(Graphics* g)
{
    ...
    else
    {
        ...
        if (mSeedType == SeedType::SEED_CONCEPTPLANT && mShootingCounter > 0)
        {
            SexyString aTimerStr = StrFormat(_S("%d"), (mShootingCounter + 99) / 100);
            int aPosX = (int)mX + mWidth / 2 - 115;
            int aPosY = (int)mY - 230;

            if (FONT_BRIANNETOD32 && FONT_BRIANNETOD32BLACK)
            {
                // Manual outline: draw black text shifted in 4 directions
                TodDrawString(g, aTimerStr, aPosX - 2, aPosY, FONT_BRIANNETOD32BLACK, Color::Black, DS_ALIGN_CENTER);
                TodDrawString(g, aTimerStr, aPosX + 2, aPosY, FONT_BRIANNETOD32BLACK, Color::Black, DS_ALIGN_CENTER);
                TodDrawString(g, aTimerStr, aPosX, aPosY - 2, FONT_BRIANNETOD32BLACK, Color::Black, DS_ALIGN_CENTER);
                TodDrawString(g, aTimerStr, aPosX, aPosY + 2, FONT_BRIANNETOD32BLACK, Color::Black, DS_ALIGN_CENTER);
                // Main white text
                TodDrawString(g, aTimerStr, aPosX, aPosY, FONT_BRIANNETOD32, Color::White, DS_ALIGN_CENTER);
            }
            else
            {
                TodDrawString(g, aTimerStr, aPosX, aPosY, Sexy::FONT_CONTINUUMBOLD14, Color::White, DS_ALIGN_CENTER);
            }
        }
...
void Plant::MouseDown(int x, int y, int theClickCount)
{
    ...
    if (mSeedType == SeedType::SEED_CONCEPTPLANT && theClickCount > 0 && mShootingCounter == 0)
    {
    ...
    case SeedType::SEED_CONCEPTPLANT:
    {
        ...
        for (int dc = -1; dc <= 1; dc++)
        {
            for (int dr = -1; dr <= 1; dr++)
            {
                if (dc == 0 && dr == 0)
                    continue;
                int aCol = mPlantCol + dc;
                int aRow = mRow + dr;
                if (aCol < 0 || aCol >= MAX_GRID_SIZE_X || aRow < 0 || aRow > MAX_GRID_SIZE_Y)
                    continue;
                Plant* aNeighbor = mBoard->GetTopPlantAt(aCol, aRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
                if (aNeighbor && aNeighbor->mSubclass == PlantSubClass::SUBCLASS_SHOOTER)
                    aNeighbor->mBoostCounter = BOOST_DURATION;
            }
        }
        mShootingCounter = 3000;
        break;
    }
    }
}
