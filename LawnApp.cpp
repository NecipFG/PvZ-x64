...
void LawnApp::Init()
{
	...
	if (fLog) { fprintf(fLog, "LawnApp::Init started\n"); fflush(fLog); }
	...
	if (fLog) { fprintf(fLog, "Calling SexyApp::Init\n"); fflush(fLog); }
	SexyApp::Init();
	if (fLog) { fprintf(fLog, "SexyApp::Init done\n"); fflush(fLog); }

	if (fLog) { fprintf(fLog, "Parsing resources.xml from: properties\\resources.xml\n"); fflush(fLog); }
	if (!mResourceManager->ParseResourcesFile("properties\\resources.xml"))
	{
		if (fLog) { fprintf(fLog, "FAILED to parse resources.xml\n"); fclose(fLog); }
		ShowResourceError(true);
		return;
	}
	if (fLog) { fprintf(fLog, "Parsing resources.xml success\n"); fflush(fLog); }

	if (fLog) { fprintf(fLog, "Loading Init group\n"); fflush(fLog); }
	if (!TodLoadResources("Init"))
	{
		if (fLog) { fprintf(fLog, "FAILED to load Init group\n"); fclose(fLog); }
		return;
	}
	if (fLog) { fprintf(fLog, "Loading Init group success\n"); fclose(fLog); }

	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "LawnApp::Init: loading profiles\n"); fclose(fLog); } }
	PerfTimer mTimer;
	mTimer.Start();

	mProfileMgr->Load();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "LawnApp::Init: loading profiles success\n"); fclose(fLog); } }
	...
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "TitleScreen created\n"); fclose(fLog); } }
	mTitleScreen->Resize(0, 0, mWidth, mHeight);
	mWidgetManager->AddWidget(mTitleScreen);
	mWidgetManager->SetFocus(mTitleScreen);
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "TitleScreen added to widget manager\n"); fclose(fLog); } }
	...
	mMusic = new Music();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "Music created\n"); fclose(fLog); } }
	mSoundSystem = new TodFoley();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "TodFoley created\n"); fclose(fLog); } }
	mEffectSystem = new EffectSystem();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "EffectSystem created\n"); fclose(fLog); } }
	mEffectSystem->EffectSystemInitialize();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "EffectSystem initialized\n"); fclose(fLog); } }
	...
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ReanimatorLoadDefinitions done\n"); fclose(fLog); } }
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_LOADBAR_SPROUT, true);
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "REANIM_LOADBAR_SPROUT ensured\n"); fclose(fLog); } }
	ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_LOADBAR_ZOMBIEHEAD, true);
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "REANIM_LOADBAR_ZOMBIEHEAD ensured\n"); fclose(fLog); } }
...
#endif
	mTimer.Start();
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "LawnApp::Init returning\n"); fclose(fLog); } }
}
...
void LawnApp::LoadingThreadProc()
{
	...
	FrameworkLog
("LawnApp::LoadingThreadProc: Loading resource groups");
	...
	FrameworkLog
("LawnApp::LoadingThreadProc: Initializing Music");
	mMusic->MusicInit();
	int aDuration = max(aTimer.GetDuration(), 0);
	aTimer.Start();

	FrameworkLog
("LawnApp::LoadingThreadProc: Creating Subsystems");
	...
	FrameworkLog
("LawnApp::LoadingThreadProc: Loading Trail definitions");
...
bool LawnApp::IsChallengeWithoutSeedBank()
{
	return 
		...
		mGameMode == GameMode::GAMEMODE_CHALLENGE_WALL_KNIGHT || 
