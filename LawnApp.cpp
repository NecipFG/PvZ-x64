...
void LawnApp::DoConfirmBackToMain() {
  LawnDialog *aDialog = (LawnDialog *)DoDialog(
      Dialogs::DIALOG_CONFIRM_BACK_TO_MAIN, true,
      _S("Leave Game?" /*"[LEAVE_GAME]"*/),
      _S("Do you want to return\nto the main menu?\n\nYour game will be "
         "saved." /*"[LEAVE_GAME_HEADER]"*/),
...
void LawnApp::DoNewOptions(bool theFromGameSelector) {
  ...
  CenterDialog(aDialog, IMAGE_OPTIONS_MENUBACK->mWidth,
               IMAGE_OPTIONS_MENUBACK->mHeight);
  AddDialog(Dialogs::DIALOG_NEWOPTIONS, aDialog);
  mWidgetManager->SetFocus(aDialog);
}

// 0x450180
AlmanacDialog *LawnApp::DoAlmanacDialog(SeedType theSeedType,
...
void LawnApp::DoPauseDialog() {
  ...
  LawnDialog *aDialog = (LawnDialog *)DoDialog(
      Dialogs::DIALOG_PAUSED, true, _S("GAME PAUSED" /*"[GAME_PAUSED]"*/),
      _S("Click to resume game"), _S("Resume Game" /*"[RESUME_GAME]"*/),
      Dialog::BUTTONS_FOOTER);

  aDialog->mReanimation->AddReanimation(
      72.0f, 42.0f, ReanimationType::REANIM_ZOMBIE_NEWSPAPER);
...
// 0x4504B0
int LawnApp::LawnMessageBox(int theDialogId, const SexyChar *theHeaderName,
  ...
  LawnDialog *aDialog =
      (LawnDialog *)DoDialog(theDialogId, true, theHeaderName, theLinesName,
...
// 0x450770
Dialog *LawnApp::DoDialog(int theDialogId, bool isModal,
  ...
  Dialog *aDialog = SexyAppBase::DoDialog(theDialogId, isModal, aHeader, aLines,
                                          aFooter, theButtonMode);
...
Dialog *LawnApp::DoDialogDelay(int theDialogId, bool isModal,
                               const SexyString &theDialogHeader,
                               const SexyString &theDialogLines,
                               const SexyString &theDialogFooter,
                               int theButtonMode) {
  LawnDialog *aDialog = (LawnDialog *)SexyAppBase::DoDialog(
      theDialogId, isModal, theDialogHeader, theDialogLines, theDialogFooter,
      theButtonMode);
...
void LawnApp::FinishUserDialog(bool isYes) {
  ...
  if (aUserDialog) {
    if (isYes) {
      PlayerInfo *aProfile = mProfileMgr->GetProfile(
          StringToSexyStringFast(aUserDialog->GetSelName()));
      if (aProfile) {
...
void LawnApp::FinishCreateUserDialog(bool isYes) {
  NewUserDialog *aNewUserDialog =
      (NewUserDialog *)GetDialog(Dialogs::DIALOG_CREATEUSER);
  ...
  if (isYes && aName.empty()) {
    DoDialog(Dialogs::DIALOG_CREATEUSERERROR, true, _S("Enter Your Name"),
             _S("Please enter your name to create a new user profile for "
                "storing high score data and game progress"),
             _S("OK"), Dialog::BUTTONS_FOOTER);
  } else if (mPlayerInfo == nullptr && (!isYes || aName.empty())) {
    DoDialog(
        Dialogs::DIALOG_CREATEUSERERROR, true,
        _S("Enter Your Name" /*"[ENTER_YOUR_NAME]"*/),
        _S("Please enter your name to create a new user profile for storing "
           "high score data and game progress" /*"[ENTER_NEW_USER]"*/),
    ...
    if (aProfile == nullptr) {
      DoDialog(Dialogs::DIALOG_CREATEUSERERROR, true,
               _S("Name Conflict" /*"[NAME_CONFLICT]"*/),
               _S("The name you entered is already being used.  Please enter a "
                  "unique player name" /*"[ENTER_UNIQUE_PLAYER_NAME]"*/),
               _S("OK" /*"[DIALOG_BUTTON_OK]"*/), Dialog::BUTTONS_FOOTER);
    } else {
      mProfileMgr->Save();
      mPlayerInfo = aProfile;
...
void LawnApp::DoConfirmDeleteUserDialog(const SexyString &theName) {
  ...
  DoDialog(
      Dialogs::DIALOG_CONFIRMDELETEUSER, true,
      _S("Are You Sure" /*"[ARE_YOU_SURE]"*/),
      // StrFormat(TodStringTranslate(_S("[DELETE_USER_WARNING]")).c_str(),
      // StringToSexyStringFast(theName))
      StrFormat(_S("This will permanently remove '%s' from the player roster!"),
                theName.c_str()),
      _S(""), Dialog::BUTTONS_YES_NO);
}
...
void LawnApp::FinishRenameUserDialog(bool isYes) {
  ...
  NewUserDialog *aNewUserDialog =
      (NewUserDialog *)GetDialog(Dialogs::DIALOG_RENAMEUSER);
  ...
  if (!mProfileMgr->RenameProfile(anOldName, aNewName)) {
    DoDialog(Dialogs::DIALOG_RENAMEUSERERROR, true,
             _S("Name Conflict" /*"[NAME_CONFLICT]"*/),
             _S("The name you entered is already being used.  Please enter a "
                "unique player name" /*"[ENTER_UNIQUE_PLAYER_NAME]"*/),
             _S("OK" /*"[DIALOG_BUTTON_OK]"*/), Dialog::BUTTONS_FOOTER);
...
void LawnApp::DoConfirmSellDialog(const SexyString &theMessage) {
  Dialog *aConfirmDialog =
      DoDialog(Dialogs::DIALOG_ZEN_SELL, true, _S("[ZEN_SELL_HEADER]"),
               theMessage, _S(""), Dialog::BUTTONS_YES_NO);
  aConfirmDialog->mYesButton->mLabel =
      TodStringTranslate(_S("[DIALOG_BUTTON_YES]"));
  aConfirmDialog->mNoButton->mLabel =
      TodStringTranslate(_S("[DIALOG_BUTTON_NO]"));
}

void LawnApp::DoConfirmPurchaseDialog(const SexyString &theMessage) {
  LawnDialog *aComfirmDialog = (LawnDialog *)DoDialog(
      Dialogs::DIALOG_STORE_PURCHASE, true, _S("买下这个物品？"), theMessage,
      _S(""), Dialog::BUTTONS_YES_NO);
  aComfirmDialog->mLawnYesButton->mLabel =
      TodStringTranslate(_S("[DIALOG_BUTTON_YES]"));
  aComfirmDialog->mLawnNoButton->mLabel =
      TodStringTranslate(_S("[DIALOG_BUTTON_NO]"));
}

// 0x451580
Dialog *LawnApp::NewDialog(int theDialogId, bool isModal,
                           const SexyString &theDialogHeader,
                           const SexyString &theDialogLines,
                           const SexyString &theDialogFooter,
                           int theButtonMode) {
  LawnDialog *aDialog = new LawnDialog(
      this, theDialogId, isModal, SexyStringToStringFast(theDialogHeader),
...
bool LawnApp::KillNewOptionsDialog() {
  NewOptionsDialog *aNewOptionsDialog =
      (NewOptionsDialog *)GetDialog(Dialogs::DIALOG_NEWOPTIONS);
...
bool LawnApp::NeedPauseGame() {
  ...
  if (mDialogList.size() == 1 &&
      mDialogList.front()->mId != Dialogs::DIALOG_NEW_GAME) {
    int anId = mDialogList.front()->mId;
    if (anId == Dialogs::DIALOG_CHOOSER_WARNING ||
        anId == Dialogs::DIALOG_PURCHASE_PACKET_SLOT ||
...
// 0x451880
void LawnApp::Init() {
  FILE *fLog = fopen("init_log.txt", "w");
  if (fLog) {
    fprintf(fLog, "LawnApp::Init started\n");
    fflush(fLog);
  }
  ...
  if (fLog) {
    fprintf(fLog, "Calling SexyApp::Init\n");
    fflush(fLog);
  }
  SexyApp::Init();
  if (fLog) {
    fprintf(fLog, "SexyApp::Init done\n");
    fflush(fLog);
  }

  if (fLog) {
    fprintf(fLog, "Parsing resources.xml from: properties\\resources.xml\n");
    fflush(fLog);
  }
  if (!mResourceManager->ParseResourcesFile("properties\\resources.xml")) {
    if (fLog) {
      fprintf(fLog, "FAILED to parse resources.xml\n");
      fclose(fLog);
    }
  ...
  if (fLog) {
    fprintf(fLog, "Parsing resources.xml success\n");
    fflush(fLog);
  }

  if (fLog) {
    fprintf(fLog, "Loading Init group\n");
    fflush(fLog);
  }
  if (!TodLoadResources("Init")) {
    if (fLog) {
      fprintf(fLog, "FAILED to load Init group\n");
      fclose(fLog);
    }
    return;
  }
  if (fLog) {
    fprintf(fLog, "Loading Init group success\n");
    fclose(fLog);
  }

  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "LawnApp::Init: loading profiles\n");
      fclose(fLog);
    }
  }
  PerfTimer mTimer;
  ...
  mProfileMgr->Load();
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "LawnApp::Init: loading profiles success\n");
      fclose(fLog);
    }
  }
  ...
  mTitleScreen = new TitleScreen(this);
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "TitleScreen created\n");
      fclose(fLog);
    }
  }
  ...
  mWidgetManager->SetFocus(mTitleScreen);
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "TitleScreen added to widget manager\n");
      fclose(fLog);
    }
  }
  ...
  mEffectSystem = new EffectSystem();
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "EffectSystem created\n");
      fclose(fLog);
    }
  }
  mEffectSystem->EffectSystemInitialize();
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "EffectSystem initialized\n");
      fclose(fLog);
    }
  }
  ...
  ReanimatorLoadDefinitions(gLawnReanimationArray,
                            ReanimationType::NUM_REANIMS);
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "ReanimatorLoadDefinitions done\n");
      fclose(fLog);
    }
  }
  ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_LOADBAR_SPROUT,
                                   true);
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "REANIM_LOADBAR_SPROUT ensured\n");
      fclose(fLog);
    }
  }
  ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_LOADBAR_ZOMBIEHEAD,
                                   true);
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "REANIM_LOADBAR_ZOMBIEHEAD ensured\n");
      fclose(fLog);
    }
  }
  ...
  mTimer.Start();
  {
    FILE *fLog = fopen("init_log.txt", "a");
    if (fLog) {
      fprintf(fLog, "LawnApp::Init returning\n");
      fclose(fLog);
    }
  }
}
  ...
  if (IsAdventureMode()) {
    ...
    if (mBoard->IsFinalSurvivalStage()) {
      ...
      if (aUnlockedNewChallenge && HasFinishedAdventure()) {
        int aNumTrophies =
            GetNumTrophies(ChallengePage::CHALLENGE_PAGE_SURVIVAL);
    ...
    if (!HasFinishedAdventure() &&
        (mGameMode == GameMode::GAMEMODE_SCARY_POTTER_3 ||
...
void LawnApp::CheckForGameEnd() {
  ...
  if (IsAdventureMode()) {
    ...
    if (IsFirstTimeAdventureMode() && aLevel < FINAL_LEVEL) {
...
void LawnApp::UpdatePlayTimeStats() {
  ...
  if (mPlayerInfo && !mPlayerInfo->mHasUsedCheatKeys && !mDebugKeysEnabled &&
      mTodCheatKeys) {
...
void LawnApp::LoadGroup(const char *theGroupName, int theGroupAveMsToLoad) {
  ...
  while (!mShutdown && !mCloseRequest && !mLoadingFailed &&
         TodLoadNextResource()) {
  ...
  if (mResourceManager->HadError() ||
      !ExtractResourcesByName(mResourceManager, theGroupName)) {
    ShowResourceError();
    mLoadingFailed = true;
  }

  int aTotalGroupWeight =
      mResourceManager->GetNumResources(theGroupName) * theGroupAveMsToLoad;
  int aGroupTime = max(aTimer.GetDuration(), 0);
  TraceLoadGroup(theGroupName, aGroupTime, aTotalGroupWeight,
                 theGroupAveMsToLoad);
}
...
void LawnApp::LoadingThreadProc() {
  FrameworkLog("LawnApp::LoadingThreadProc: Started\n");
  if (!TodLoadResources("LoaderBar")) {
    FrameworkLog("LawnApp::LoadingThreadProc: Failed to load LoaderBar\n");
    return;
  }

  FrameworkLog("LawnApp::LoadingThreadProc: Loading LawnStrings.txt\n");
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Precalculating tasks\n");
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Loading resource groups");
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Initializing Music");
  mMusic->MusicInit();
  int aDuration = max(aTimer.GetDuration(), 0);
  aTimer.Start();

  FrameworkLog("LawnApp::LoadingThreadProc: Creating Subsystems");
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Loading Trail definitions");
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Loading Particle definitions");
  TodParticleLoadDefinitions(gLawnParticleArray, LENGTH(gLawnParticleArray));
  aDuration = max(aTimer.GetDuration(), 0);
  aTimer.Start();

  FrameworkLog("LawnApp::LoadingThreadProc: Preloading for user");
  PreloadForUser();
  if (mLoadingFailed || mShutdown || mCloseRequest)
    return;
  ...
  FrameworkLog("LawnApp::LoadingThreadProc: Loading Sound group");
  GetNumPreloadingTasks();
  LoadGroup("LoadingSounds", 54);
  TodHesitationTrace("finished loading");
  FrameworkLog("LawnApp::LoadingThreadProc: Done");
}
...
void LawnApp::URLOpenFailed(const std::string &theURL) {
  ...
  std::string aString = "Please open the following URL in your browser\n\n" +
                        theURL +
                        "\n\nFor your convenience, this URL has already been "
                        "copied to your clipboard.";

  DoDialog(Dialogs::DIALOG_OPENURL_WAIT, true, _S("Open Browser"), _S("OK"),
           StringToSexyStringFast(aString), Dialog::BUTTONS_FOOTER);
}
...
bool LawnApp::OpenURL(const std::string &theURL, bool shutdownOnOpen) {
  DoDialog(Dialogs::DIALOG_OPENURL_WAIT, true, _S("Opening Browser"),
           _S("Opening Browser"), _S(""), Dialog::BUTTONS_NONE);
...
void LawnApp::ConfirmQuit() {
  ...
  LawnDialog *aDialog =
      (LawnDialog *)DoDialog(Dialogs::DIALOG_QUIT, true, aHeader, aBody, _S(""),
...
void LawnApp::ButtonDepress(int theId) {
  if (theId % 10000 >= 2000 &&
      theId % 10000 < 3000) // 按钮编号 theId ∈ [2000, 3000) 时，表示按下 theId
                            // - 2000 编号的对话中的“是”按钮
  {
    switch (theId - 2000) {
  ...
  if (theId % 10000 >= 3000 &&
      theId < 4000) // 按钮编号 theId ∈ [3000, 4000) 时，表示按下 theId - 3000
                    // 编号的对话中的“否”按钮
  {
    switch (theId - 3000) {
    case Dialogs::DIALOG_PREGAME_NAG:
...
void LawnApp::CenterDialog(Dialog *theDialog, int theWidth, int theHeight) {
  theDialog->Resize((BOARD_WIDTH - theWidth) / 2,
                    (BOARD_HEIGHT - theHeight) / 2, theWidth, theHeight);
}
...
SexyString LawnApp::GetStageString(int theLevel) {
  int aArea =
      ClampInt((theLevel - 1) / LEVELS_PER_AREA + 1, 1, ADVENTURE_AREAS + 1);
...
bool LawnApp::IsFinalBossLevel() {
  ...
  return IsAdventureMode() &&
         (mPlayerInfo->mLevel == 50 || mPlayerInfo->mLevel == FINAL_LEVEL);
}
...
bool LawnApp::IsChallengeWithoutSeedBank() {
  return mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS ||
         mGameMode == GameMode::GAMEMODE_UPSELL ||
         mGameMode == GameMode::GAMEMODE_INTRO || IsWhackAZombieLevel() ||
         IsSquirrelLevel() || IsScaryPotterLevel() ||
         mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN ||
...
PottedPlant *LawnApp::GetPottedPlantByIndex(int thePottedPlantIndex) {
  TOD_ASSERT(thePottedPlantIndex >= 0 &&
             thePottedPlantIndex < mPlayerInfo->mNumPottedPlants);
...
SeedType LawnApp::GetAwardSeedForLevel(int theLevel) {
  ...
  int aSeedsHasGot =
      (aArea - 1) * 8 +
      aSub; // 一般来说，每大关可以获得 8 种植物，每小关可以获得 1 种植物
  if (aSub >= 10) {
...
int LawnApp::GetSeedsAvailable() {
  ...
  if (HasFinishedAdventure() || aLevel > 50) {
    return (int)NUM_SEEDS_IN_CHOOSER;
  }

  SeedType aSeedTypeMax = GetAwardSeedForLevel(aLevel);
  return min((int)NUM_SEEDS_IN_CHOOSER, (int)aSeedTypeMax);
}
...
bool LawnApp::HasSeedType(SeedType theSeedType) {
  ...
  /*  优化
  if (theSeedType >= SeedType::SEED_TWINSUNFLOWER && theSeedType <=
  SeedType::SEED_IMITATER) return mPlayerInfo->mPurchases[theSeedType -
  SeedType::SEED_GATLINGPEA];
  */

  if (theSeedType == SeedType::SEED_TWINSUNFLOWER) {
    return mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_TWINSUNFLOWER] > 0;
  }
  if (theSeedType == SeedType::SEED_GLOOMSHROOM) {
    return mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_GLOOMSHROOM] > 0;
  }
  if (theSeedType == SeedType::SEED_CATTAIL) {
    return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_CATTAIL] >
           0;
  }
  if (theSeedType == SeedType::SEED_WINTERMELON) {
    return mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_WINTERMELON] > 0;
  }
  if (theSeedType == SeedType::SEED_GOLD_MAGNET) {
    return mPlayerInfo
               ->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_GOLD_MAGNET] > 0;
  }
  if (theSeedType == SeedType::SEED_SPIKEROCK) {
    return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_SPIKEROCK] >
           0;
  }
  if (theSeedType == SeedType::SEED_COBCANNON) {
    return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_COBCANNON] >
           0;
  }
  if (theSeedType == SeedType::SEED_IMITATER) {
    return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_IMITATER] >
           0;
  }
...
Reanimation *LawnApp::AddReanimation(float theX, float theY, int theRenderOrder,
  ...
  return mEffectSystem->mReanimationHolder->AllocReanimation(
      theX, theY, theRenderOrder, theReanimationType);
}

// 0x453C80
TodParticleSystem *LawnApp::AddTodParticle(float theX, float theY,
                                           int theRenderOrder,
                                           ParticleEffect theEffect) {
  return mEffectSystem->mParticleHolder->AllocParticleSystem(
      theX, theY, theRenderOrder, theEffect);
}

ParticleSystemID LawnApp::ParticleGetID(TodParticleSystem *theParticle) {
  return (ParticleSystemID)mEffectSystem->mParticleHolder->mParticleSystems
      .DataArrayGetID(theParticle);
}

ReanimationID LawnApp::ReanimationGetID(Reanimation *theReanimation) {
  return (ReanimationID)mEffectSystem->mReanimationHolder->mReanimations
      .DataArrayGetID(theReanimation);
}
...
bool LawnApp::AdvanceCrazyDaveText() {
  SexyString aMessageName =
      StrFormat(_S("[CRAZY_DAVE_%d]"), mCrazyDaveMessageIndex + 1);
...
SexyString LawnApp::GetCrazyDaveText(int theMessageIndex) {
  ...
  aMessage =
      TodReplaceString(aMessage, _S("{PLAYER_NAME}"), mPlayerInfo->mName);
  aMessage = TodReplaceString(aMessage, _S("{MONEY}"),
                              GetMoneyString(mPlayerInfo->mCoins));
  int aCost = StoreScreen::GetItemCost(StoreItem::STORE_ITEM_PACKET_UPGRADE);
  aMessage =
      TodReplaceString(aMessage, _S("{UPGRADE_COST}"), GetMoneyString(aCost));
  return aMessage;
}
...
bool LawnApp::CanShowStore() {
  ...
  return HasFinishedAdventure() || mPlayerInfo->mHasSeenUpsell ||
         mPlayerInfo->mLevel >= 25;
}
...
bool LawnApp::CanSpawnYetis() {
  const ZombieDefinition &aZombieDef =
      GetZombieDefinition(ZombieType::ZOMBIE_YETI);
...
bool LawnApp::HasBeatenChallenge(GameMode theGameMode) {
  ...
  int aChallengeIndex =
      theGameMode - GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1;
  TOD_ASSERT(aChallengeIndex >= 0 && aChallengeIndex < NUM_CHALLENGE_MODES);
  if (IsSurvivalNormal(theGameMode)) {
    return mPlayerInfo->mChallengeRecords[aChallengeIndex] >=
...
void LawnApp::CrazyDaveEnter() {
  ...
  aCrazyDaveReanim->PlayReanim(
      "anim_enter", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 24.0f);
...
void LawnApp::CrazyDaveLeave() {
  ...
  if (aCrazyDaveReanim) {
    ...
    aCrazyDaveReanim->PlayReanim(
        "anim_leave", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 24.0f);
...
void LawnApp::CrazyDaveTalkMessage(const SexyString &theMessage) {
  ...
  if (mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_TALKING || doSound) {
    if (doHanding) {
      aCrazyDaveReanim->PlayReanim("anim_talk_handing",
                                   ReanimLoopType::REANIM_LOOP, 50, 12.0f);
      ...
      aCrazyDaveReanim->PlayReanim(
          "anim_crazy", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 50, 12.0f);
      ...
      aWallnutReanim->PlayReanim("anim_idle", ReanimLoopType::REANIM_LOOP, 0,
                                 12.0f);
      TodTrace("Handed");

      ReanimatorTrackInstance *aHandTrackInstance =
          aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
      AttachEffect *aAttachEffect = AttachReanim(
          aHandTrackInstance->mAttachmentID, aWallnutReanim, 100.0f, 393.0f);
      ...
      aHammerReanim->PlayReanim("anim_whack_zombie",
                                ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0,
                                24.0f);
      ...
      ReanimatorTrackInstance *aHandTrackInstance =
          aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
      AttachEffect *aAttachEffect = AttachReanim(
          aHandTrackInstance->mAttachmentID, aHammerReanim, 62.0f, 445.0f);
      ...
      Reanimation *aFertilizerReanim = AddReanimation(
          0.0f, 0.0f, 0, ReanimationType::REANIM_ZENGARDEN_FERTILIZER);
      aFertilizerReanim->PlayReanim(
          "bag", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 24.0f);
      aFertilizerReanim->mAnimRate = 0.0f;

      ReanimatorTrackInstance *aHandTrackInstance =
          aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
      AttachReanim(aHandTrackInstance->mAttachmentID, aFertilizerReanim, 102.0f,
                   412.0f);
      aCrazyDaveReanim->Update();
      ...
      Reanimation *aTreeFoodReanim = AddReanimation(
          0.0f, 0.0f, 0, ReanimationType::REANIM_TREEOFWISDOM_TREEFOOD);
      aTreeFoodReanim->PlayReanim(
          "bag", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 24.0f);
      aTreeFoodReanim->mAnimRate = 0.0f;

      ReanimatorTrackInstance *aHandTrackInstance =
          aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
      AttachReanim(aHandTrackInstance->mAttachmentID, aTreeFoodReanim, 102.0f,
                   412.0f);
      aCrazyDaveReanim->Update();
      ...
      Reanimation *aMoneyBagReanim = AddReanimation(
          0.0f, 0.0f, 0, ReanimationType::REANIM_ZENGARDEN_FERTILIZER);
      aMoneyBagReanim->PlayReanim(
          "bag", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 0, 24.0f);
      ...
      ReanimatorTrackInstance *aHandTrackInstance =
          aCrazyDaveReanim->GetTrackInstanceByName("Dave_handinghand");
      AttachReanim(aHandTrackInstance->mAttachmentID, aMoneyBagReanim, 90.0f,
                   405.0f);
      aCrazyDaveReanim->Update();
      /*
...
void LawnApp::CrazyDaveStopTalking() {
  ...
  if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_HANDING_TALKING &&
      !aDoneHanding) {
    aCrazyDaveReanim->PlayReanim("anim_idle_handing",
                                 ReanimLoopType::REANIM_LOOP, 20, 12.0f);
    mCrazyDaveState = CrazyDaveState::CRAZY_DAVE_HANDING_IDLING;
  } else if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_TALKING ||
             mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_HANDING_TALKING) {
    aCrazyDaveReanim->PlayReanim("anim_idle", ReanimLoopType::REANIM_LOOP, 20,
                                 12.0f);
...
void LawnApp::UpdateCrazyDave() {
  ...
  if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_IDLING ||
    ...
    if (mCrazyDaveMessageText.find("{MOUTH_BIG_SMILE}") != std::string::npos) {
      aCrazyDaveReanim->SetImageOverride("Dave_mouths",
                                         IMAGE_REANIM_CRAZYDAVE_MOUTH1);
    } else if (mCrazyDaveMessageText.find("{MOUTH_SMALL_SMILE}") !=
               std::string::npos) {
      aCrazyDaveReanim->SetImageOverride("Dave_mouths",
                                         IMAGE_REANIM_CRAZYDAVE_MOUTH5);
    } else if (mCrazyDaveMessageText.find("{MOUTH_BIG_OH}") !=
               std::string::npos) {
      aCrazyDaveReanim->SetImageOverride("Dave_mouths",
                                         IMAGE_REANIM_CRAZYDAVE_MOUTH4);
    } else if (mCrazyDaveMessageText.find("{MOUTH_SMALL_OH}") !=
               std::string::npos) {
      aCrazyDaveReanim->SetImageOverride("Dave_mouths",
  ...
  if (mCrazyDaveState == CrazyDaveState::CRAZY_DAVE_IDLING ||
    ...
    if (mCrazyDaveBlinkCounter <= 0) {
      ...
      aBlinkReanim->mLoopType =
          ReanimLoopType::REANIM_PLAY_ONCE_FULL_LAST_FRAME_AND_HOLD;
      ...
      aCrazyDaveReanim->AssignRenderGroupToTrack("Dave_eye",
                                                 RENDER_GROUP_HIDDEN);
...
void LawnApp::DrawCrazyDave(Graphics *g) {
  ...
  if (mCrazyDaveMessageText.size()) {
    ...
    if (clickToContinue) {
      TodDrawString(g, _S("click to continue"), aPosX + 139, aPosY + 140,
                    FONT_PICO129, Color::Black,
...
int LawnApp::GetNumPreloadingTasks() {
  ...
  if (mPlayerInfo) {
    ...
    for (ZombieType i = ZombieType::ZOMBIE_NORMAL;
         i < ZombieType::NUM_ZOMBIE_TYPES; i = (ZombieType)((int)i + 1)) {
      if (HasFinishedAdventure() ||
...
void LawnApp::PreloadForUser() {
  ...
  ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_LAWN_MOWERED_ZOMBIE,
                                   true);
  ...
  mCompletedLoadingThreadTasks += 68;
  ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_ZOMBIE_NEWSPAPER,
                                   true);
  mCompletedLoadingThreadTasks += 68;
  ReanimatorEnsureDefinitionLoaded(ReanimationType::REANIM_SELECTOR_SCREEN,
                                   true);
  ...
  if (mPlayerInfo) {
    for (SeedType i = SeedType::SEED_PEASHOOTER; i < SeedType::NUM_SEED_TYPES;
      ...
      if (SeedTypeAvailable(i) || HasFinishedAdventure()) {
        ...
        if (mTitleScreen &&
            mTitleScreen->mQuickLoadKey != KeyCode::KEYCODE_UNKNOWN) {
          TodTrace("preload canceled\n");
          mNumLoadingThreadTasks = aNumTasks;
          return;
        }
...
// 0x455AA0
SexyString LawnApp::Pluralize(int theCount, const SexyChar *theSingular,
                              const SexyChar *thePlural) {
  if (theCount == 1) {
...
int LawnApp::TrophiesNeedForGoldSunflower() {
  return 48 - GetNumTrophies(CHALLENGE_PAGE_SURVIVAL) -
         GetNumTrophies(CHALLENGE_PAGE_CHALLENGE) -
...
void LawnApp::InitHook() {
  ...
  mDRM = new PopDRMComm();
  mDRM->DoIPC();
  if (sexystricmp(GetString("MarketingMode", _S("")).c_str(),
                  _S("StageLocked")) == 0) {
...
SexyString LawnApp::GetMoneyString(int theAmount) {
  ...
  if (aValue > 999999) {
    return StrFormat(_S("$%d,%03d,%03d"), aValue / 1000000,
                     (aValue - aValue / 1000000 * 1000000) / 1000,
                     aValue - aValue / 1000 * 1000);
  } else if (aValue > 9999) {
    return StrFormat(_S("$%d,%03d"), aValue / 1000,
                     aValue - aValue / 1000 * 1000);
  } else {
...
SexyString LawnGetCurrentLevelName() {
  ...
  if (gLawnApp->IsAdventureMode()) {
    return StrFormat(
        _S("F%d"), gLawnApp->GetStageString(gLawnApp->mBoard->mLevel).c_str());
  }
...
bool LawnApp::CanDoPinataMode() {
  ...
  return mPlayerInfo->mChallengeRecords
             [(int)GameMode::GAMEMODE_TREE_OF_WISDOM -
...
void LawnApp::BetaAddFile(std::list<std::string> &theUploadFileList,
                          std::string theFileName, std::string theShortName) {}

void LawnApp::TraceLoadGroup(const char *theGroupName, int theGroupTime,
                             int theTotalGroupWeigth, int theTaskWeight) {}
