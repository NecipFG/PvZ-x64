...
static PakInterfaceBase* GetPakPtr()
{
	...
	{
		char aName[256];
		sprintf(aName, "gPakInterfaceP_%d", (int)GetCurrentProcessId());
		...
		if (fLog) { fprintf(fLog, "GetPakPtr: Initialized mapping %s, gPakInterfaceP=%p\n", aName, gPakInterfaceP); fclose(fLog); }
	}
	return (gPakInterfaceP != NULL) ? *gPakInterfaceP : NULL;
#else
	static PakInterfaceBase* aPakInterface = nullptr;
	if (gPakInterfaceP == nullptr)
	{
		gPakInterfaceP = &aPakInterface;
	}
	return aPakInterface;
#endif
}
...
{
	if (theFile == NULL || theFile->mFP == NULL)
		return 0;
