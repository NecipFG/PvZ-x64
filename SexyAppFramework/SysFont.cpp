...
{
	mApp = gSexyAppBase;
	mHeight = thePointSize;
	mAscent = thePointSize * 0.8;
	mDrawShadow = false;
	mSimulateBold = false;
	mHFont = nullptr;
}
...
int	SysFont::StringWidth(const SexyString& theString)
{
	return theString.length() * (mHeight * 0.6);
}

void SysFont::DrawString(Graphics* g, int theX, int theY, const SexyString& theString, const Color& theColor, const Rect& theClipRect)
{
	// GDI font drawing is not used on Linux since all game text is ImageFont-based.
}
