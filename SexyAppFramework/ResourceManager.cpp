...
{
	std::lock_guard<std::recursive_mutex> aLock(mResMutex);
...
{
	std::lock_guard<std::recursive_mutex> aLock(mResMutex);
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ResourceManager::ParseResourcesFile: %s\n", theFilename.c_str()); fclose(fLog); } }
	mXMLParser = new XMLParser();
	if (!mXMLParser->OpenFile(theFilename))
	{
		{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ResourceManager::ParseResourcesFile: OpenFile FAILED\n"); fclose(fLog); } }
		Fail("Resource file not found: " + theFilename);
		return false;
	}
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ResourceManager::ParseResourcesFile: OpenFile SUCCESS\n"); fclose(fLog); } }
	...
	while (!mXMLParser->HasFailed())
	{
		if (!mXMLParser->NextElement(&aXMLElement))
		{
			{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ResourceManager::ParseResourcesFile: NextElement FAILED: %s\n", SexyStringToStringFast(mXMLParser->GetErrorText()).c_str()); fclose(fLog); } }
			Fail(SexyStringToStringFast(mXMLParser->GetErrorText()));
			break;
		}
		...
		if (aXMLElement.mType == XMLElement::TYPE_START)
		{
			...
			{
				{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "ResourceManager::ParseResourcesFile: Calling DoParseResources\n"); fclose(fLog); } }
	...
	for (int i=0; i < aNumRows; i++)
	{
		...
		for (int j=0; j < aNumCols; j++)
		{
			...
			for (int y=0; y<aCelHeight; y++)
			{
				...
				{
					*aDestPtr = (*aDestPtr & 0x00FFFFFF) | ((ulong)(*anAlphaBits & 0xFF) << 24);
					++anAlphaBits;
					++aDestPtr;
				}
...
bool ResourceManager::LoadAlphaImage(ImageRes *theRes, DDImage *theImage)
{
	...
	{
		*aBits1 = (*aBits1 & 0x00FFFFFF) | ((ulong)(*aBits2 & 0xFF) << 24);
		++aBits1;
		++aBits2;
	}
...
{
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "DoLoadImage: %s\n", theRes->mId.c_str()); fclose(fLog); } }
