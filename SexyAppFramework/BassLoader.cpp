...
#define GetProcAddress(h, x) dlsym(h, x)
#define FreeLibrary(h) dlclose(h)
#define InterlockedIncrement(x) __sync_add_and_fetch(x, 1)
#define InterlockedDecrement(x) __sync_sub_and_fetch(x, 1)
#define BASS_DLL_NAME "libbass.so"
#else
#define BASS_DLL_NAME ".\\bass.dll"
#endif
...
BOOL BASS_INSTANCE::BASS_MusicPlayEx(HMUSIC handle, DWORD pos, int flags, BOOL reset)
{
	...
	BASS_ChannelSetPosition(handle, (QWORD)(pos & ~0x80000000), 1 /*BASS_POS_MUSIC_ORDER*/);
