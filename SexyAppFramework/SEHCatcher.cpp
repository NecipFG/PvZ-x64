...
long __stdcall SEHCatcher::UnhandledExceptionFilter(LPEXCEPTION_POINTERS lpExceptPtr)
{
	...
	if (inFilter)
	{
		fprintf(stderr, "Recursive crash detected in exception filter! Exiting immediately.\n");
		fflush(stderr);
		ExitProcess(1);
	}
	inFilter = true;
...
void SEHCatcher::DoHandleDebugEvent(LPEXCEPTION_POINTERS lpEP)
{
	...
	aDebugDump += "\r\n";
#ifdef _WIN64
	sprintf(aBuffer, ("RAX:%016I64X RBX:%016I64X RCX:%016I64X RDX:%016I64X RSI:%016I64X RDI:%016I64X\r\n"),
...
std::string SEHCatcher::ImageHelpWalk(PCONTEXT theContext, int theSkipCount)
{
	...
	for (;;)
	{
		if (!mStackWalk(machineType, GetCurrentProcess(), GetCurrentThread(),
						&sf, theContext, NULL, mSymFunctionTableAccess, mSymGetModuleBase, 0))
		{
