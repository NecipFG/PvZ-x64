...
static void InitAppDataFolder()
{
...
#ifdef _WIN32
	char aPath[MAX_PATH];
	if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, 0, aPath)))
	{
		Sexy::gAppDataFolder = std::string(aPath) + "\\PopCap Games\\PlantsVsZombies\\";
	}
	else
	{
		const char* appDataEnv = getenv("APPDATA");
		if (appDataEnv != nullptr && strlen(appDataEnv) > 0)
		{
			Sexy::gAppDataFolder = std::string(appDataEnv) + "\\PopCap Games\\PlantsVsZombies\\";
		}
		else
		{
			Sexy::gAppDataFolder = "";
	...
	if (xdgDataHome != nullptr && strlen(xdgDataHome) > 0)
	{
		Sexy::gAppDataFolder = std::string(xdgDataHome) + "/PlantsVsZombies/";
	}
	else
	{
		...
		{
			Sexy::gAppDataFolder = std::string(home) + "/.local/share/PlantsVsZombies/";
		}
		else
		{
			Sexy::gAppDataFolder = "";
...
bool Sexy::FileExists(const std::string& theFileName)
{
	...
	{
#ifdef _WIN32
		std::string fn = theFileName;
#else
		std::string fn = sexy_resolve_path(theFileName);
#endif
