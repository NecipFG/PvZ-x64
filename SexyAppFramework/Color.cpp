...
Color::Color(int theColor) :
	mAlpha(((uint32_t)theColor >> 24) & 0xFF),
	mRed(((uint32_t)theColor   >> 16) & 0xFF),
	mGreen(((uint32_t)theColor >> 8 ) & 0xFF),
	mBlue(((uint32_t)theColor       ) & 0xFF)
{
	if(mAlpha==0)
		mAlpha = 0xff;
}
...
{
	return ((uint32_t)mAlpha << 24) | ((uint32_t)mRed << 16) | ((uint32_t)mGreen << 8) | ((uint32_t)mBlue);
}
