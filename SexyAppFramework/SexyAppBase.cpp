...
namespace Sexy
{
	...
	static std::map<std::string, RegVal> gRegistryMap;
	static std::recursive_mutex gRegistryMutex;
	static bool gRegistryLoaded = false;

	static std::string GetRegistryFilePath()
	{
		return GetAppDataFolder() + "userdata/registry.txt";
	}

	static std::string StringToHex(const uchar* data, size_t len)
	{
	...
	static std::vector<uchar> HexToString(const std::string& hex)
	{
		...
		for (size_t i = 0; i < hex.length(); i += 2)
		{
			...
			uchar byte = (uchar)strtol(byteString.c_str(), nullptr, 16);
			bytes.push_back(byte);
		}
		return bytes;
	}

	static void LoadRegistrySim()
	{
		std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
		if (gRegistryLoaded) return;
		gRegistryLoaded = true;

		std::string aPath = GetRegistryFilePath();
		...
		while (std::getline(file, line))
		{
			...
			ulong type = (ulong)std::stoul(line.substr(0, space1));

			size_t eq = line.find('=', space1 + 1);
			if (eq == std::string::npos) continue;

			std::string key = line.substr(space1 + 1, eq - space1 - 2);
			while (!key.empty() && isspace((unsigned char)key.back())) key.pop_back();
			
			std::string valHex = line.substr(eq + 2);
			while (!valHex.empty() && isspace((unsigned char)valHex.back())) valHex.pop_back();
			while (!valHex.empty() && isspace((unsigned char)valHex.front())) valHex.erase(valHex.begin());

			gRegistryMap[key] = { type, HexToString(valHex) };
		}
	}

	static void SaveRegistrySim()
	{
		std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
		std::string aPath = GetRegistryFilePath();
		try {
			std::filesystem::create_directories(std::filesystem::path(aPath).parent_path());
		} catch (...) {}
		std::ofstream file(aPath);
		if (!file.is_open()) return;

		for (const auto& pair : gRegistryMap)
		{
			file << pair.second.type << " " << pair.first << " = " << StringToHex(pair.second.value.data(), pair.second.value.size()) << "\n";
		}
	}
}

static void HighPrecisionSleep(int theMilliseconds)
{
	if (theMilliseconds <= 0)
		return;
	DWORD aSleepStart = timeGetTime();
	if (theMilliseconds > 2)
	{
		Sleep(theMilliseconds - 2);
	}
	while ((int)(timeGetTime() - aSleepStart) < theMilliseconds)
	{
		Sleep(0);
	}
}

void FrameworkLog(const char* theFormat, ...)
{
	va_list argList;
	va_start(argList, theFormat);
	vfprintf(stderr, theFormat, argList);
	va_end(argList);
	fflush(stderr);
}
...
HMODULE Sexy::gDDrawDLL = NULL;
HMODULE Sexy::gDSoundDLL = NULL;
...
SexyAppBase::SexyAppBase()
{
	...
	mPrimaryThreadId = std::thread::id();
...
void SexyAppBase::TakeScreenshot()
{
	...
	try
	{
		for (const auto& entry : std::filesystem::directory_iterator(anImageDir))
		{
			if (entry.is_regular_file())
			{
				std::string aFileName = entry.path().filename().string();
				int aNum = 0;
				if (sscanf(aFileName.c_str(), (anImagePrefix + "%d.png").c_str(), &aNum) == 1)
				{
					if (aNum > aMaxId)
						aMaxId = aNum;
				}
...
bool SexyAppBase::RegistryWrite(const std::string& theValueName, ulong theType, const uchar* theValue, ulong theLength)
{
	...
	std::string fullKey = aKeyName + "\\" + (aSlashPos != -1 ? theValueName.substr(aSlashPos + 1) : theValueName);

	{
		std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
		LoadRegistrySim();
		std::vector<uchar> valBytes(theValue, theValue + theLength);
		gRegistryMap[fullKey] = { theType, valBytes };
		SaveRegistrySim();
	}
...
bool SexyAppBase::RegistryEraseKey(const SexyString& _theKeyName)
{
	...
	std::string aKeyPrefix = RemoveTrailingSlash("SOFTWARE\\" + mRegKey) + "\\" + theKeyName;

	{
		std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
		LoadRegistrySim();
		
		for (auto it = gRegistryMap.begin(); it != gRegistryMap.end(); )
		{
			if (it->first == aKeyPrefix || it->first.rfind(aKeyPrefix + "\\", 0) == 0)
			{
				it = gRegistryMap.erase(it);
			}
			else
			{
				++it;
...
bool SexyAppBase::RegistryGetSubKeys(const std::string& theKeyName, StringVector* theSubKeys)
{
	...
	else
	{
		...
		std::set<std::string> uniqueSubKeys;
		{
			std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
			LoadRegistrySim();
			for (const auto& pair : gRegistryMap)
			{
				if (pair.first.rfind(aKeyName, 0) == 0)
				{
					std::string suffix = pair.first.substr(aKeyName.length());
					size_t nextSlash = suffix.find('\\');
					if (nextSlash != std::string::npos)
					{
						uniqueSubKeys.insert(suffix.substr(0, nextSlash));
					}
				}
			}
		}

		for (const auto& subKey : uniqueSubKeys)
		{
			theSubKeys->push_back(subKey);
		}
...
bool SexyAppBase::RegistryReadKey(const std::string& theValueName, ulong* theType, uchar* theValue, ulong* theLength, HKEY theKey)
{
	...
	{		
		...
		RegVal regVal;
		{
			std::lock_guard<std::recursive_mutex> lock(gRegistryMutex);
			LoadRegistrySim();
			auto it = gRegistryMap.find(fullKey);
			if (it != gRegistryMap.end())
			{
				found = true;
				regVal = it->second;
			}
		...
		{
			*theType = regVal.type;
			ulong copyLen = std::min(*theLength, (ulong)regVal.value.size());
			*theLength = regVal.value.size();
			if (theValue != nullptr && copyLen > 0)
			{
				std::copy(regVal.value.begin(), regVal.value.begin() + copyLen, theValue);
			}
...
std::string SexyAppBase::GetGameSEHInfo()
{
	...
	aStream << mPrimaryThreadId;
	std::string aThreadIdStr = aStream.str();
...
{
	if ((mPrimaryThreadId != std::thread::id()) && (std::this_thread::get_id() != mPrimaryThreadId))
	{
		mLoadingFailed = true;
	}
	else if (!mShutdown)
...
void SexyAppBase::UpdateFrames()
{
	...
	if (!mMinimized)
	{
		FrameworkLog("SexyAppBase::UpdateFrames: Updating Widgets\n");
		if (mWidgetManager->UpdateFrame())
			++mFPSDirtyCount;
	}

	FrameworkLog("SexyAppBase::UpdateFrames: Updating Music\n");
	mMusicInterface->Update();
	FrameworkLog("SexyAppBase::UpdateFrames: Cleaning Shared Images\n");
	CleanSharedImages();
	FrameworkLog("SexyAppBase::UpdateFrames: Done\n");
}
...
void SexyAppBase::Redraw(Rect* theClipRect)
{
	...
	static DWORD aRetryTick = 0;
	{
		...
		if (fLog)
		{
			fprintf(fLog, "SexyAppBase::Redraw: this=%p, gSexyAppBase=%p, mDDInterface=%p\n", this, gSexyAppBase, mDDInterface);
			fclose(fLog);
		}
	}
...
bool SexyAppBase::DrawDirtyStuff()
{
	...
	static int sDrawDirtyStuffCount = 0;
	if (sDrawDirtyStuffCount < 100)
	{
		sDrawDirtyStuffCount++;
		FrameworkLog("DrawDirtyStuff: Call %d, drewScreen=%d, aStartTime=%u, mLastDrawTick=%u, diff=%u, mCustomCursorDirty=%d, mNextDrawTick=%u, next_diff=%d\n",
			sDrawDirtyStuffCount, drewScreen, aStartTime, mLastDrawTick, aStartTime - mLastDrawTick, mCustomCursorDirty, mNextDrawTick, (int)(aStartTime - mNextDrawTick));
	}
		...
		((int) (aStartTime - mNextDrawTick) >= 0))
	{
		...
		if (mWaitForVSync && mIsPhysWindowed && mSoftVSyncWait)
		{
			...
			if (aTick-mLastDrawTick < mDDInterface->mMillisecondsPerFrame)
				HighPrecisionSleep(mDDInterface->mMillisecondsPerFrame - (aTick-mLastDrawTick));
		}
...
static INT_PTR CALLBACK MarkerListDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			HDC hDCListBox = GetDC(aListBox);
			HFONT hFontNew = (HFONT)SendMessage(aListBox, WM_GETFONT, NULL, NULL);
...
#else
static int ListDemoMarkers() { return -1; }
#endif
...
#else
static int DemoJumpToTime() { return -1; }
#endif
...
{
	// Log only some messages to avoid spam
	if (uMsg != WM_MOUSEMOVE && uMsg != WM_TIMER && uMsg != WM_PAINT)
	{
		FrameworkLog("SexyAppBase::WindowProc: hWnd=%p uMsg=0x%X wParam=0x%llX lParam=0x%llX\n", hWnd, uMsg, (unsigned long long)wParam, (unsigned long long)lParam);
	}
...
#else
	SexyAppBase* aSexyApp = gSexyAppBase;
#endif
	if (uMsg != WM_MOUSEMOVE && uMsg != WM_TIMER && uMsg != WM_PAINT)
	{
		FrameworkLog("SexyAppBase::WindowProc: aSexyApp=%p (gSexyAppBase=%p)\n", aSexyApp, gSexyAppBase);
	}
...
/*	case WM_DISPLAYCHANGE:
	...
	case WM_DESTROY:
		...
		break;	
	case WM_SETCURSOR:
		if (aSexyApp != NULL && !aSexyApp->mSEHOccured)
	...
	case WM_ENDSESSION:
		...
		break;
	case WM_PAINT:
		if (aSexyApp != NULL && (aSexyApp->mInitialized) && (!gInAssert) && (!aSexyApp->mSEHOccured))
		{
			RECT aClientRect;
...
void SexyAppBase::MakeWindow()
{
	...
	if ((mPlayingDemoBuffer) || (mIsWindowed && !mFullScreenWindow))
	{
		...
		aDesktopRect.left = 0;
		aDesktopRect.top = 0;
		aDesktopRect.right = 1920;
		aDesktopRect.bottom = 1080;
...
{
	FrameworkLog("SexyAppBase::LoadingThreadProcStub: Started\n");
	...
	FrameworkLog("SexyAppBase::LoadingThreadProcStub: %s", aStr);

	FrameworkLog("SexyAppBase::LoadingThreadProcStub: Done\n");
...
void SexyAppBase::CursorThreadProc()
{
	...
	while (!mShutdown)
	{
		if (mDDInterface == NULL) { Sleep(10); continue; }
...
void SexyAppBase::StartCursorThread()
{
	if (!mCursorThreadRunning)
	{
		...
		::SetThreadPriority(::GetCurrentThread(), THREAD_PRIORITY_ABOVE_NORMAL);
		std::thread(CursorThreadProcStub, this).detach();
	}
}
...
bool SexyAppBase::Process(bool allowSleep)
{
	...
	if ((!mPaused) && (mUpdateMultiplier > 0))
	{
		...
		{
			if ((++mNonDrawCount < (int) ceil(10*mUpdateMultiplier)) || (!mLoaded && mNonDrawCount < 100))
			{
				bool doUpdate = false;
		...
		{			
			...
			else
			{
				...
				if (aTimeToNextFrame > 0)
				{
					...
					++mSleepCount;
					DWORD aSleepStart = timeGetTime();
					HighPrecisionSleep(aTimeToNextFrame);

					aCumSleepTime += (int)(timeGetTime() - aSleepStart);					
				}
#else
				int aTimeToNextFrame = (int) (aFrameFTime - mUpdateFTimeAcc);
				if (aTimeToNextFrame > 0)
				{
					if (!allowSleep)
						return false;
					++mSleepCount;
					std::this_thread::sleep_for(std::chrono::milliseconds(aTimeToNextFrame));
				}
#endif
			}
		}
...
void SexyAppBase::DoMainLoop()
{
	FrameworkLog("SexyAppBase::DoMainLoop: Entry\n");
	while (!mShutdown)
	{
		if (mExitToTop)
			mExitToTop = false;
...
static WPARAM MapSDLKToVK(SDL_Keycode sym)
{
	...
	switch (sym)
	{
		...
		case SDLK_PAUSE:     return 0x13; // VK_PAUSE
		...
		case SDLK_PRINTSCREEN: return 0x2C; // VK_SNAPSHOT
...
bool SexyAppBase::UpdateAppStep(bool* updated)
{
	...
	{		
		FrameworkLog("SexyAppBase::UpdateAppStep: Processing Messages\n");
		
		SDL_Event event;
		while (SDL_PollEvent(&event) && (!mShutdown))
		{
#ifdef _WIN32
			if (event.type == SDL_QUIT)
			{
				WindowProc(mHWnd, WM_CLOSE, 0, 0);
			}
#else
			switch (event.type)
			{
				...
				case SDL_WINDOWEVENT:
					if (event.window.event == SDL_WINDOWEVENT_FOCUS_GAINED)
						WindowProc(mHWnd, WM_ACTIVATEAPP, TRUE, 0);
					else if (event.window.event == SDL_WINDOWEVENT_FOCUS_LOST)
						WindowProc(mHWnd, WM_ACTIVATEAPP, FALSE, 0);
					break;

				case SDL_MOUSEMOTION:
					WindowProc(mHWnd, WM_MOUSEMOVE, 0, (event.motion.y << 16) | (event.motion.x & 0xFFFF));
					break;
				...
				case SDL_MOUSEBUTTONUP:
				{
					...
					{
						if (down)
							msg = (event.button.clicks > 1) ? WM_LBUTTONDBLCLK : WM_LBUTTONDOWN;
						else
							msg = WM_LBUTTONUP;
					}
					...
					{
						if (down)
							msg = (event.button.clicks > 1) ? WM_RBUTTONDBLCLK : WM_RBUTTONDOWN;
						else
							msg = WM_RBUTTONUP;
					}
					...
					if (msg != 0)
						WindowProc(mHWnd, msg, 0, (event.button.y << 16) | (event.button.x & 0xFFFF));
					break;
				}
				...
				case SDL_MOUSEWHEEL:
				{
					...
					WindowProc(mHWnd, WM_MOUSEWHEEL, (event.wheel.y * 120) << 16, (my << 16) | (mx & 0xFFFF));
					break;
				}
				...
				case SDL_TEXTINPUT:
				{
					...
					{
						WindowProc(mHWnd, WM_CHAR, (unsigned char)event.text.text[i], 0);
					}
					break;
				}
			}
	...
	{
		FrameworkLog("SexyAppBase::UpdateAppStep: Processing State %d\n", mUpdateAppState);
...
void SexyAppBase::Start()
{
	...
	FrameworkLog("SexyAppBase::Start: Starting cursor thread\n");
	StartCursorThread();

	if (mAutoStartLoadingThread)
	{
		FrameworkLog("SexyAppBase::Start: Starting loading thread\n");
		StartLoadingThread();
	}

	FrameworkLog("SexyAppBase::Start: Showing window\n");
	...
	FrameworkLog("SexyAppBase::Start: Entering DoMainLoop\n");
	DoMainLoop();
	ProcessSafeDeleteList();
	mRunning = false;
...
{
	{ FILE* fLog = fopen("framework_init_log.txt", "a"); if (fLog) { fprintf(fLog, "DoParseCmdLine starting...\n"); fclose(fLog); } }
	...
	mCmdLineParsed = true;
	{ FILE* fLog = fopen("framework_init_log.txt", "a"); if (fLog) { fprintf(fLog, "DoParseCmdLine done.\n"); fclose(fLog); } }
}
...
{
	std::string aDir = ".";
	std::string aPrefix = theDemoPrefix;
	size_t aLastSlash = theDemoPrefix.find_last_of("\\/");
	if (aLastSlash != std::string::npos)
	{
		aDir = theDemoPrefix.substr(0, aLastSlash);
		aPrefix = theDemoPrefix.substr(aLastSlash + 1);
	}
	...
	try
	{
		for (const auto& entry : std::filesystem::directory_iterator(aDir))
		{
			if (entry.is_regular_file())
			{
				std::string aFileName = entry.path().filename().string();
				int aNum = 0;
				if (sscanf(aFileName.c_str(), (aPrefix + "%d.dmo").c_str(), &aNum) == 1)
					aSet.insert(aNum);
			}
		}
	}
	...
	{
		std::string aFileToErase = StrFormat((theDemoPrefix + "%d.dmo").c_str(), *anItr);
		std::filesystem::remove(aFileToErase);
	}
...
{
	FILE* fLog = fopen("framework_init_log.txt", "w");
	if (fLog) { fprintf(fLog, "SexyAppBase::Init started\n"); fflush(fLog); }
	mPrimaryThreadId = std::this_thread::get_id();
	if (mShutdown) return;
	if (fLog) { fprintf(fLog, "Checking DLLs\n"); fflush(fLog); }
	...
	{
		mIsWideWindow = false;
		WNDCLASSA wc; memset(&wc, 0, sizeof(wc)); wc.lpfnWndProc = WindowProc; wc.hInstance = gHInstance; wc.lpszClassName = "MainWindow"; RegisterClassA(&wc);
		wc.lpszClassName = "InvisWindow"; RegisterClassA(&wc);
		mInvisHWnd = CreateWindowExA(0, "InvisWindow", "Invis", 0, 0, 0, 0, 0, NULL, NULL, gHInstance, NULL);
	}
	else
	{
		mIsWideWindow = sizeof(SexyChar) == sizeof(wchar_t);
		WNDCLASSW wc; memset(&wc, 0, sizeof(wc)); wc.lpfnWndProc = WindowProc; wc.hInstance = gHInstance; wc.lpszClassName = L"MainWindow"; RegisterClassW(&wc);
		wc.lpszClassName = L"InvisWindow"; RegisterClassW(&wc);
		mInvisHWnd = CreateWindowExW(0, L"InvisWindow", L"Invis", 0, 0, 0, 0, 0, NULL, NULL, gHInstance, NULL);
	}
	if (mInvisHWnd) SetWindowLongPtr(mInvisHWnd, GWLP_USERDATA, (LONG_PTR) this);
	...
	InitHook();
	mInitialized = true;
	if (fLog) { fprintf(fLog, "SexyAppBase::Init SUCCESS\n"); fclose(fLog); }
}
...
{
	return mDDInterface && mDDInterface->mD3DTester && mDDInterface->mD3DTester->Is3DSupported();
}

bool SexyAppBase::Is3DAccelerationRecommended()
{
	return mDDInterface && mDDInterface->mD3DTester && mDDInterface->mD3DTester->Is3DRecommended();
}
...
static CritSect gSharedImageCritSect;

SharedImageRef SexyAppBase::GetSharedImage(const std::string& theFileName, const std::string& theVariant, bool* isNew)
{
	{ FILE* fLog = fopen("init_log.txt", "a"); if (fLog) { fprintf(fLog, "GetSharedImage: %s\n", theFileName.c_str()); fclose(fLog); } }
	...
	{
		AutoCrit anAutoCrit(gSharedImageCritSect);
