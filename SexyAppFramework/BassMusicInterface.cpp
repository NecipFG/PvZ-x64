...
BassMusicInterface::BassMusicInterface(HWND theHWnd)
{
	...
	{ FILE* fLog = fopen("framework_init_log.txt", "a"); if (fLog) { fprintf(fLog, "BassMusicInterface: calling BASS_Init with hWnd=%p\n", theHWnd); fclose(fLog); } }
	success = gBass->BASS_Init(-1, 44100, 0, theHWnd, NULL);
	{ FILE* fLog = fopen("framework_init_log.txt", "a"); if (fLog) { fprintf(fLog, "BassMusicInterface: BASS_Init result=%d\n", success); fclose(fLog); } }
	if (gBass->mVersion2)
	{
#ifndef __linux__
		gBass->BASS_SetConfig(BASS_CONFIG_BUFFER, 2000);
#else
		gBass->BASS_SetConfig(BASS_CONFIG_BUFFER, 500);
#endif
	}
...
bool BassMusicInterface::LoadMusic(int theSongId, const std::string& theFileName)
{
	...
	else
	{
		...
		aHMusic = gBass->BASS_MusicLoad(FALSE, (void*) theFileName.c_str(), 0, 0, BASS_MUSIC_LOOP | (gBass->mVersion2 ? BASS2_MUSIC_RAMP : 0), 0);
...
void BassMusicInterface::PlayMusic(int theSongId, int theOffset, bool noLoop)
{
	...
	if (anItr != mMusicMap.end())
	{
		...
		gBass->BASS_ChannelSetAttribute(aMusicInfo->GetHandle(), 2 /*BASS_ATTRIB_VOL*/, (float)aMusicInfo->mVolume);
		...
		else
		{
			...
			if (theOffset > 0)
				gBass->BASS_ChannelSetPosition(aMusicInfo->mHStream, (QWORD)theOffset, 0 /*BASS_POS_BYTE*/);
		}
	}
}
...
int BassMusicInterface::GetMusicOrder(int theSongId)
{
	...
	{		
		BassMusicInfo* aMusicInfo = &anItr->second;
		int aPosition = (int)gBass->BASS_ChannelGetPosition(aMusicInfo->GetHandle(), 1 /*BASS_POS_MUSIC_ORDER*/);
		return aPosition;
	}
	return -1;
}