...
int DDInterface::Init(HWND theWindow, bool IsWindowed)
{
	...
	if (!gSDL2Renderer)
	{
		gSDL2Renderer = new SDL2RendererWrapper();
		if (!gSDL2Renderer->Init(theWindow, mWidth, mHeight, IsWindowed))
		{
			delete gSDL2Renderer;
			gSDL2Renderer = nullptr;
		}
	}

	if (gSDL2Renderer)
	{
		mRefreshRate = gSDL2Renderer->GetTargetFPS();
		mMillisecondsPerFrame = 1000 / mRefreshRate;
		...
		mScreenImage->mNoLock = false;
		mScreenImage->mVideoMemory = false;
...
bool DDInterface::Redraw(Rect* theClipRect)
{
	...
	if (gSDL2Renderer)
	{
		gSDL2Renderer->BeginFrame();
		gSDL2Renderer->Blt(mScreenImage, 0, 0, Rect(0, 0, mWidth, mHeight), Color::White, 0);
		if (mCursorImage != nullptr)
		{
			int curX = mNextCursorX - mCursorImage->mWidth / 2;
			int curY = mNextCursorY - mCursorImage->mHeight / 2;
			gSDL2Renderer->Blt(mCursorImage, curX, curY, Rect(0, 0, mCursorImage->mWidth, mCursorImage->mHeight), Color::White, 0);
		}
		gSDL2Renderer->Present();
		return true;
	}
...
bool DDInterface::Do3DTest(HWND)
{
	return true;
}
...
bool DDInterface::CopyBitmap(LPDIRECTDRAWSURFACE, HBITMAP, int, int, int, int)
{
	return true;
}

void DDInterface::AddDDImage(DDImage*) {}
void DDInterface::RemoveDDImage(DDImage*) {}
void DDInterface::Remove3DData(MemoryImage*) {}
void DDInterface::RestoreOldCursorAreaFrom(LPDIRECTDRAWSURFACE, bool) {}
void DDInterface::DrawCursorTo(LPDIRECTDRAWSURFACE, bool) {}
void DDInterface::MoveCursorTo(LPDIRECTDRAWSURFACE, bool, int, int) {}
HRESULT DDInterface::CreateSurface(DDSURFACEDESC2*, LPDIRECTDRAWSURFACE*, void*) { return 0; }
void DDInterface::ClearSurface(LPDIRECTDRAWSURFACE) {}
void DDInterface::SetVideoOnlyDraw(bool) {}
#endif
