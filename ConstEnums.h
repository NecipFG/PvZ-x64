...
enum Dialogs {
  ...
  DIALOG_CONTINUE,        // 35：继续游戏（读档进入关卡时）
  DIALOG_GETREADY,        // 36：
  DIALOG_RESTARTCONFIRM, // 37：重新开始游戏（继续游戏对话中选择开始新游戏时触发）
  DIALOG_CONFIRMPURCHASE, // 38：
  DIALOG_CONFIRMSELL,     // 39：
  DIALOG_TIMESUP,         // 40：
  DIALOG_VIRTUALHELP,     // 41：
  DIALOG_JUMPAHEAD,       // 42：
  DIALOG_CRAZY_DAVE,      // 43：
  DIALOG_STORE_PURCHASE,  // 44：购买商品（戴夫商店）
  DIALOG_ZEN_SELL,        // 45：出售盆栽植物（禅境花园）
  DIALOG_MESSAGE,         // 46：通用消息对话
  DIALOG_IMITATER,        // 47：模仿者对话
  DIALOG_PURCHASE_PACKET_SLOT, // 48：升级卡槽的格子数量
  DIALOG_HOTKEY_HELP,          // 49：快捷键帮助
  NUM_DIALOGS
};
enum DebugTextMode {
  DEBUG_TEXT_NONE = 0,
