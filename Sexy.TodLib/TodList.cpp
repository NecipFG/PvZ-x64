...
{
	// Align theItemSize to a multiple of sizeof(void*) to prevent unaligned write/read hazards on 64-bit systems
	int align = sizeof(void*);
	theItemSize = (theItemSize + align - 1) & ~(align - 1);
...
void TodAllocator::Grow()
{
	...
	*(void**)aBlock = mBlockList;
	...
	void* aItem = (void*)((intptr_t)aBlock + sizeof(void*));
	for (int i = 0; i < mGrowCount; i++)
	{
		*(void**)aItem = aFreeList;
		aFreeList = aItem;
		aItem = (void*)((intptr_t)aItem + mItemSize);
	}
	mFreeList = aFreeList;
}
...
bool TodAllocator::IsPointerFromAllocator(void* theItem)
{
	...
	{
		intptr_t aItemPtr = (intptr_t)theItem;
		// 区块的首个指针大小区域为额外申请的、用于存储指向下一区块的指针的区域
		intptr_t aBlockPtr = (intptr_t)aPtr + sizeof(void*);
		// 判断 theItem 是否位于当前区块内且指向某一项的区域的起始地址
		if (aItemPtr >= aBlockPtr && aItemPtr < aBlockPtr + (intptr_t)aBlockSize && (aItemPtr - aBlockPtr) % mItemSize == 0)
			return true;
	}
	return false;
}
