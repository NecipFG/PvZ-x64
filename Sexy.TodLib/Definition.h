...
public:
    ...
    int                 mFieldOffset;                   //+0x4：_MemVar 在所处类中的偏移量（DISK offset）
    int                 mMemoryOffset;                  // MEMORY offset (offsetof)
...
public:
    ...
    int                 mDefSize;                       //+0x4：一个 _DefClass 实例在 DISK 上的大小
    int                 mMemorySize;                    // 一个 _DefClass 实例在 MEMORY 上的大小 (sizeof)
