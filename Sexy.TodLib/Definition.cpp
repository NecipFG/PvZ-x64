...
bool DefReadFromCacheArray(void*& theReadPtr, DefinitionArrayDef* theArray, DefMap* theDefMap)
{
    ...
    for (int i = 0; i < theArray->mArrayCount; i++)
    {
        ...
        if (!DefMapReadFromCache(theReadPtr, theDefMap, aElemDest))
            return false;
    }
    return true;
}
...
{
    void* aTemp32Buffer = _alloca(theDefMap->mDefSize);
    SMemR(theReadPtr, aTemp32Buffer, theDefMap->mDefSize);

    for (DefField* aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++)
    {
        bool aSucceed = true;
        void* aSrc = (void*)((intptr_t)aTemp32Buffer + aField->mFieldOffset);
