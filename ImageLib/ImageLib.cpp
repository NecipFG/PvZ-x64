...
Image* GetTGAImage(const std::string& theFileName)
{
	...
	anImage->mBits = new ulong[anImageWidth*anImageHeight];
...
Image* GetGIFImage(const std::string& theFileName)
{
	...
	*/
	...
	for (; ; )
	{
		...
		ulong* colortable = new ulong[colors];  // ÉêÇëÑÕÉ«ÁÐ±í
...
Image* GetJPEGImage(const std::string& theFileName)
{
	...
	ulong* aBits = new ulong[cinfo.output_width*cinfo.output_height];
	ulong* q = aBits;
...
#if 1  // Use open-source JasPer sources (j2k-codec.dll path disabled for MinGW build)
Image* GetJPEG2000Image(const std::string& theFileName)
{
	...
	anImage->mBits = new ulong[aMaxWidth * aMaxHeight];
	...
	for (i=0; i<aNumComponents; i++)
	{
		...
		ulong* destRow = anImage->mBits + yorig*numVSteps*anImage->GetWidth() + xorig*numHSteps;
		...
		for (int y=0; y<numVSteps; y++)
		{
			...
			for (int x=0; x<numHSteps; x++)
			{
				...
				ulong *destRowWriter = dest;
				for (int j=0; j<vstep; j++)
				{
					ulong *destWriter = destRowWriter;
...
{
	// jas_init() is called lazily inside GetJPEG2000Image()
}
...
Image* GetJPEG2000Image(const std::string& theFileName)
{
	if (gJ2KCodec != NULL)
	{
		...
		ulong* aBuffer = new ulong[aWidth*aHeight];
		if (aBuffer == NULL)
		{
			(*fJ2K_Close)(aJ2KImage);
			return NULL;
